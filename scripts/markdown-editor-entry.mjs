...
export async function createScalAIMarkdownEditor(root, markdown, options = {}) {
  ...
  const builder = new CrepeBuilder({ root, defaultValue: encodeCellBreaks(markdown) })
    ...
    .addFeature(blockEdit, {
      ...
      // video creation stay out of the menu until ScalAI has one complete flow.
      advancedGroup: { image: null, math: null },
    })
    .addFeature(placeholder, { text: 'Type / for blocks', mode: 'block' })
    .addFeature(toolbar, {
      buildToolbar: (groups) => {
        groups.addGroup('nami-formatting', 'ScalAI formatting')
          .addItem('highlight', {
