// Serve a pretend newer ScalAI from this machine, so the updater can be tested
...
//
//   1. npm run pack                      → release/mac-arm64/ScalAI.app  (the old one)
//   2. cp -R release/mac-arm64/ScalAI.app /Applications/ScalAI-test.app
//   3. bump version in package.json, npm run pack   → the new one
//   4. node scripts/fake-update.mjs --serve release/mac-arm64/ScalAI.app \
//                                   --point /Applications/ScalAI-test.app
//   5. open /Applications/ScalAI-test.app, click download, quit, reopen
//
...
if (!serveApp && !pointApp) {
  console.error('usage: node scripts/fake-update.mjs --serve <ScalAI.app> [--point <installed ScalAI.app>] [--corrupt]');
  process.exit(1);
}
...
if (serveApp) {
  ...
  const zip = path.join(OUT, `ScalAI-${process.arch}.zip`);
...
if (pointApp) {
  ...
  if (!fs.existsSync(res)) {
    console.error(`no Contents/Resources inside ${target} — is that a packaged ScalAI?`);
    process.exit(1);
  }
