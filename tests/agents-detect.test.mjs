...
const { KNOWN_AGENTS, POINTER_FILE, contextFichiersFor, detectAgents, pathFromShellOutput, findOnDisk } = require('../src/main/agents-detect.js');
...
test('contextFichiersFor returns AGENTS.md plus a stub only where one is needed', () => {
  assert.deepEqual(contextFichiersFor(['codex']), ['AGENTS.md']);
  assert.deepEqual(contextFichiersFor(['codex', 'hermes', 'kimi', 'opencode']), ['AGENTS.md']);
  assert.deepEqual(contextFichiersFor(['claude', 'codex', 'antigravity']).sort(), ['AGENTS.md', 'CLAUDE.md', 'GEMINI.md']);
  assert.deepEqual(contextFichiersFor([]), ['AGENTS.md']);   // the block still has a home
  assert.deepEqual(contextFichiersFor(), ['AGENTS.md']);
});

test('only agents with a verified project skills dir claim native registration', () => {
  const native = KNOWN_AGENTS.filter((a) => a.projectCompétencesDir).map((a) => a.id);
  assert.deepEqual(native, ['claude']);
  assert.equal(KNOWN_AGENTS.find((a) => a.id === 'claude').projectCompétencesDir, '.claude/skills');
});
...
test('every lifecycle that can sign in can also sign out', () => {
  for (const a of KNOWN_AGENTS) {
    ...
    assert.ok(lc.statusCmd || (lc.statusFichiers && lc.statusFichiers.length), `${a.id} has no way to read status`);
    if (lc.statusFichiers) for (const p of lc.statusFichiers) assert.ok(p.startsWith('~/'), `${a.id}: ${p} must be ~-relative`);
