...
// suffix check can match on every platform.
...
before(() => {
  ...
  // the project's own neutral folder — the only place ScalAI writes
...
// the rail said "Compétences · 0" with total confidence.
...
test('createItem scaffolds a claude project agent and refuses overwrite', () => {
  ...
  assert.ok(res.ok);
  assert.ok(posix(res.filePath).endsWith('.claude/agents/my-new-agent.md'));
...
test('createItem scaffolds a skill in the project\'s own folder, and an opencode agent', () => {
  ...
  assert.ok(sk.ok);
  assert.ok(posix(sk.filePath).endsWith('skills/cool-skill/SKILL.md'), sk.filePath);
  ...
  assert.ok(oc.ok);
  assert.ok(posix(oc.filePath).endsWith('.config/opencode/agent/oc-agent.md'));
...
test('"Use here" copies a skill into the project\'s own folder, -copy on collision', () => {
  ...
  assert.ok(one.ok);
  assert.ok(posix(one.filePath).endsWith('skills/tdd/SKILL.md'), one.filePath);
  ...
  assert.ok(two.ok);
  assert.ok(posix(two.filePath).endsWith('skills/tdd-copy/SKILL.md'), two.filePath);
});
...
test('duplicateItem copies a plugin agent file into the project', () => {
  ...
  assert.ok(res.ok);
  assert.ok(posix(res.filePath).endsWith('.claude/agents/critic.md'));
});
