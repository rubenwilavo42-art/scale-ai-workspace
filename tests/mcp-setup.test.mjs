...
test('catalogServices never surfaces nami-browser even if it is stuffed in', () => {
  const cat = catalogServices([
    ...
    { id: 'nami-browser', name: 'ScalAI Browser' },
    { id: 'gmail', name: 'Gmail' },
  ]);
  assert.deepEqual(cat.map((s) => s.id), ['notion', 'gmail']);
});

test('guided seed tells the agent to register connections.json mcpServers, then ScalAI delivers', () => {
  ...
  assert.match(s, /ScalAI copies it to every installed agent's own config/);
...
test('catalog html hides nami-browser and keeps click-to-connect copy', () => {
  const html = connectCatalogHtml({
    catalog: [
      ...
      { id: 'nami-browser', name: 'ScalAI Browser', desc: 'this Mac\'s browser', code: 'NB' },
    ],
  ...
  assert.doesNotMatch(html, /nami-browser/);
  assert.doesNotMatch(html, /ScalAI Browser/);
});
