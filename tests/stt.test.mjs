...
test('status reports every provider, and only a key saved in ScalAI counts as saved', () => {
  ...
  assert.equal(oa.keyEnregistrerd, false);    // …but the UI field stays empty, no shell talk
...
test('a key saved on the Keys page reads as saved and beats the shell export', () => {
  ...
  assert.equal(s.providers.find((p) => p.id === 'openai').keyEnregistrerd, true);
});
