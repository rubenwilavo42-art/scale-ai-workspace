...
test('real delayed worker cancels promptly, removes snapshots and releases its destination',async()=>{
  ...
  const jobs=createImportJobs({resolveSource:()=>h.source,resolveProfile:id=>({id,name:'Work'}),createWorker:data=>{const w=fixtureWorker(data,{delay:10000});workers.push(w);return w;},applyBatch:async()=>{throw Error('Annulerled import must not write');}});
