...
// tests are about the file that decides whether ScalAI can ever update itself.

function fixture(bytes, { url = 'ScalAI-1.0.0-arm64.dmg', recordedSha = 'stale', recordedSize = 1 } = {}) {
...
test('everything else in the file is left alone', () => {
  ...
  assert.equal(doc.path, 'ScalAI-1.0.0-arm64.dmg');
});
...
test('an entry for a dmg we did not staple is untouched', () => {
  ...
  doc.files.push({ url: 'ScalAI-1.0.0.dmg', sha512: 'someone-elses', size: 99 });
...
test('a missing latest-mac.yml is not an error', () => {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'nami-staple-'));
  const dmg = path.join(dir, 'ScalAI-1.0.0-arm64.dmg');
