// The shell announcing that the command ScalAI typed into it has finished.
...
test('nothing of the sequence is left visible in the output', { skip: !hasZsh }, () => {
  ...
  assert.doesNotMatch(out, /printf|033|ScalAIRunDone=%s/);
});
...
test('a one-shot is spawned with its command, so nothing is typed', () => {
  ...
  assert.match(args[2], /ScalAIRunDone/);
