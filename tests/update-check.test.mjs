...
const release = (over = {}) => ({
  ...
  draft: false,
  prerelease: false,
  html_url: 'https://github.com/example/scalai/releases/tag/v0.2.0',
  assets: [
    { name: 'ScalAI-0.2.0-arm64.dmg', browser_download_url: 'https://example.test/arm64.dmg' },
    { name: 'ScalAI-0.2.0.dmg', browser_download_url: 'https://example.test/x64.dmg' },
  ],
  ...over,
});
...
test('falls back to the release page when no dmg matches', () => {
  ...
  assert.equal(r.url, 'https://github.com/example/scalai/releases/tag/v0.2.0');
});
