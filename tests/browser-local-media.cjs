// Real ScalAI browser and saved HTML preview playback. All media and state are
...
app.whenReady().then(async () => {
  try {
    ...
    for (const clip of clips) {
      ...
      assert.equal(await page.executeJavaScript('typeof window.scalai'), 'undefined');
    }
    ...
    for (const clip of ['clip.mp4', 'clip-audio.mp4', 'clip-audio.webm']) {
      ...
      const frame = await until(() => win.webContents.mainFrame.framesInSubtree.find(f => f.url.startsWith('scalai-doc:') && f.url.endsWith('/' + clip + '.html')), 'preview frame');
