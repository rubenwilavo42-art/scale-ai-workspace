// Electron integration uses disposable ScalAI profiles and fake credentials only.
...
protocol.registerSchemesAsPrivileged([{ scheme: 'scalai-doc', privileges: { standard: true, secure: true, supportFetchAPI: true } }]);
app.whenReady().then(async () => {
  ...
  try {
    ...
    await win.loadURL('data:text/html,<p>Trusted ScalAI test window</p>');
    ...
    assert.match(a.getURL(), /^scalai-doc:/);
    assert.equal(a.session.storagePath, null);
