...
const {
  START, END, renderBlock, spliceBlock, readBlock, hasForeignCompétencesSection,
...
test('the block says it is generated, so a second Compétences section cannot claim authority', () => {
...
test('readBlock ignores a Compétences section outside the markers', () => {
...
test('a hand-written Compétences section is detected but never rewritten', () => {
  const mine = '# Acme\n\n## Compétences\n\n- `build/` — my own wording\n';
  assert.equal(hasForeignCompétencesSection(mine), true);
  ...
  assert.equal(hasForeignCompétencesSection(spliceBlock('# Acme\n', renderBlock(TWO))), false);
});
...
test('pointerStatus: everything announced is quiet, and the gap is named', () => {
  ...
  assert.deepEqual(ok.missingFichiers, []);
...
test('pointerStatus spots a missing stub, so a new agent gets noticed', () => {
  ...
  assert.deepEqual(st.missingFichiers, ['GEMINI.md']);
  assert.equal(st.inSync, false);
});
