...
test('a browser tile: Chrome, copy address, add to session, then the move items', () => {
  ...
  assert.deepEqual(labels(items), ['Ouvrir dans Chrome ↗', 'Copy address', 'Add to session', '-', 'Move to desk']);
});
...
test('a markdown tile: add to session, copy path, new window — no Chrome line', () => {
  ...
  assert.deepEqual(labels(items), ['Add to session', 'Copier le chemin', 'Ouvrir dans une nouvelle fenêtre', '-', 'Move to desk']);
});
...
test('an html tile gets the Chrome line too, after the file verbs', () => {
  ...
  assert.deepEqual(labels(items), ['Add to session', 'Copier le chemin', 'Ouvrir dans une nouvelle fenêtre', 'Ouvrir dans Chrome ↗', '-', 'Move to desk']);
});
...
test('no move items means no dangling divider', () => {
  ...
  assert.deepEqual(labels(items), ['Add to session', 'Copier le chemin', 'Ouvrir dans une nouvelle fenêtre']);
});
