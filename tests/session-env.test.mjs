...
// The bug: launch ScalAI from a terminal that is itself inside a claude session
// — `open -a ScalAI` from such a shell, an `npm start` in a claude tile — and
// every var that session exported lands in ScalAI's environment, and from there
// in every tile ScalAI spawns. claude reads CLAUDE_CODE_CHILD_SESSION, decides it
