...
function agyStore(home, cwd, id, mtime) {
  ...
  put(db, 'not really sqlite — existence and mtime are all ScalAI reads');
  setMtime(db, mtime);
}
...
test('readSessionTitle: opencode reads the title column and ignores its own placeholder', () => {
  const home = mkhome();
  opencodeStore(home, [{ id: 'ses_1', cwd: CWD, at: 5000, title: 'Rename database columns' }, { id: 'ses_2', cwd: CWD, at: 6000, title: 'Nouvelle session - 2026-09-06T07:29:09.553Z' }]);
