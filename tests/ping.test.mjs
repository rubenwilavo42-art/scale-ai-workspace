...
test('telemetry is disabled by default', () => {
  assert.equal(pingPlan(base()), null);
});

test('explicit endpoint mints an id and flags first', () => {
  const plan = pingPlan({ ...base(), env: { SCALAI_PING_URL: 'http://localhost:8788/api/ping' } });
...
test('a later launch reuses the stored id, un-flagged', () => {
  const plan = pingPlan({ ...base(), env: { SCALAI_PING_URL: 'http://localhost:8788/api/ping' }, settings: { pingId: '00000000-0000-4000-8000-000000000010' } });
...
test('a dev run sends nothing', () => {
  // Unpackaged = someone building ScalAI; their launches must not count as users.
  assert.equal(pingPlan({ ...base(), isPackaged: false }), null);
});

test('SCALAI_PING_URL explicitly enables a development ping', () => {
  const plan = pingPlan({ ...base(), isPackaged: false, env: { SCALAI_PING_URL: 'http://localhost:8788/api/ping' } });
  assert.equal(plan.url, 'http://localhost:8788/api/ping');
});
...
test('sendPing posts the payload and persists a minted id', async () => {
  ...
  const r = await sendPing({
    ...base(),
    env: { SCALAI_PING_URL: 'http://localhost:8788/api/ping' },
    saveParamètres: (patch) => saved.push(patch),
    fetchImpl: async (url, opts) => { calls.push({ url, opts }); return { ok: true }; },
  });
  assert.equal(r.sent, true);
  assert.equal(calls.length, 1);
  assert.equal(calls[0].url, 'http://localhost:8788/api/ping');
...
test('even a throwing saveParamètres cannot break the launch', async () => {
  const r = await sendPing({
    ...base(),
    env: { SCALAI_PING_URL: 'http://localhost:8788/api/ping' },
    saveParamètres: () => { throw new Error('disk full'); },
    fetchImpl: async () => ({ ok: true }),
  });
