...
test('ScalAI Browser is not a catalog entry', () => {
  assert.equal(serviceById('nami-browser'), null);
  assert.ok(!KNOWN_SERVICES.some((s) => s.id === 'nami-browser' || /nami-browser/i.test(s.name)));
});

test('guided finish contract: write connections.json mcpServers, then ScalAI delivers', () => {
  assert.match(GUIDED_FINISH, /connections\.json/);
  assert.match(GUIDED_FINISH, /mcpServers/);
  assert.match(GUIDED_FINISH, /ScalAI copies it to every installed agent's own config/);
