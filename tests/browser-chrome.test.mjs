...
import { browserParamètresContent } from '../src/renderer/browser-settings.mjs';
...
test('quiet new-tab page is blank, not a product tour', () => {
  ...
  assert.match(views, /namiThèmeIsDark/);
...
test('downloads are asked or auto-saved, never cancelled wholesale', () => {
  ...
  assert.match(views, /setEnregistrerPath/);
...
test('Paramètres → Browser is compact controls for download, popups, camera/mic and cookie import', () => {
  const html = browserParamètresContent({
  ...
  assert.match(html, /System follows ScalAI/);
...
test('plus opens a new browser tab and does not offer a companion Agent', () => {
  assert.match(pane, /title="Nouvel onglet du navigateur"/);
