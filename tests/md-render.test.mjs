...
test('setext underlines make real headings', () => {
  const html = renderMarkdown(FIXTURE);
  assert.match(html, /<h1>ScalAI 0\.9 — cards update<\/h1>/);
...
test('images render through the resolver; without one they stay links', () => {
  const withResolver = renderMarkdown(FIXTURE, { resolveImage: (src) => 'scalai-doc://doc/x/' + src });
  assert.match(withResolver, /<img src="scalai-doc:\/\/doc\/x\/shots\/wave\.png" alt="wave">/);
