...
// binds ⌘W, and ScalAI binds ⌘W to close the active *pane*. A menu accelerator
...
test('six menus on macOS, in the order a Mac user reads them', () => {
  ...
  assert.deepEqual(labels, ['ScalAI', 'File', 'Edit', 'View', 'Window', 'Help']);
});

test('off macOS there is no app submenu, so File exists to hold quit', () => {
  const win = build({ platform: 'win32' });
  assert.ok(!win.some((m) => m.label === 'ScalAI'), 'built a mac app submenu off mac');
...
test('About ScalAI opens ScalAI’s own pane, not the grey macOS panel', () => {
  const sent = [];
  const nami = build({ send: (c) => sent.push(c) }).find((m) => m.label === 'ScalAI');
  const about = nami.submenu.find((i) => i.label === 'About ScalAI');
...
test('the theme submenu is radios with the saved one ticked', () => {
  const theme = build({ theme: 'dusk' }).find((m) => m.label === 'View')
    .submenu.find((i) => i.label === 'Thème').submenu;
  assert.deepEqual(theme.map((i) => i.label), ['Clair', 'Sombre']);
  for (const item of theme) assert.equal(item.type, 'radio');
  assert.deepEqual(theme.filter((i) => i.checked).map((i) => i.label), ['Sombre']);
});

test('Open Recent is the real list, and stays away when there is none', () => {
  const sent = [];
  const recents = [{ path: '/Users/x/scalai' }, { path: '/Users/x/vault' }];
  const file = build({ recents, send: (c) => sent.push(c) }).find((m) => m.label === 'File');
  const recent = file.submenu.find((i) => i.label === 'Open Recent');
  assert.deepEqual(recent.submenu.map((i) => i.label), ['scalai', 'vault']);
...
// Eight items, seven destinations: the repo is opened by both ScalAI on GitHub
// and ★ Star ScalAI, which is deliberate. One is "read the source", the other is
...
// No telemetry anywhere in ScalAI, so the UTM is the entire measurement story:
...
// ScalAI's copy has no em dashes in it. The menu bar is the most-read copy in the
