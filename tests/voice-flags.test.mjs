...
// Paramètres · Voice said "ready" for OpenAI and ElevenLabs on a profile where
// Paramètres · Keys said "not set" for the same two keys. Both were telling the
...
// only ever shows what ScalAI itself stores.
//
...
const cloud = (over = {}) => ({
  ...
  ready: false, keyEnregistrerd: false, downloadBytes: 0, reason: null, ...over,
});

test('a key you saved in ScalAI reads as plainly ready', () => {
  assert.equal(voiceFlag(cloud({ ready: true, keyEnregistrerd: true })), 'ready');
});

test('a key that only exists in this run says where it came from', () => {
  const flag = voiceFlag(cloud({ ready: true, keyEnregistrerd: false }));
  assert.notEqual(flag, 'ready', 'an inherited key must not read the same as a saved one');
  assert.match(flag, /prêt/i, 'it is still usable right now, and should say so');
...
test('the on-device engine is never described as borrowing a key', () => {
  const local = { id: 'local', needsKey: null, keyEnregistrerd: false, downloadBytes: 0, reason: null };
...
test('the status payload still reports saved and usable separately', () => {
  ...
  assert.match(stt, /keyEnregistrerd:/, 'whether ScalAI is the one holding it');
});
