// The rule that keeps an agent-written HTML page from reaching into ScalAI.
//
// A source assertion, from a real mistake caught by hand. The page runs in an
// iframe; whether it can read ScalAI depends on the pairing of its origin and its
// sandbox flags:
//
//   file:// or srcdoc + allow-same-origin  → SAME origin as ScalAI → can read it.
...
//     the app. Forbidden.
//
//   scalai-doc:// + allow-same-origin        → the page's OWN origin, cross-origin
//     to ScalAI → cannot read it, and its relative images load. This is the safe
...
// on a scalai-doc source". Plus the served policy must forbid the network, or a
...
function iframeSandboxes(src) {
  ...
  while ((m = re.exec(src))) {
    ...
    const source = m[3] === 'docUrl' || /docUrl\(/.test(m[2]) ? 'scalai-doc'
      : m[3] ? 'file' : 'srcdoc';
  ...
  while ((m = re2.exec(src))) {
    out.push({ sandbox: m[1].trim(), source: m[2] === 'docUrl' ? 'scalai-doc' : 'file' });
  }
  return out;
}
...
test('allow-same-origin appears only on a scalai-doc source', () => {
  for (const f of frames) {
    if (f.sandbox.split(/\s+/).includes('allow-same-origin')) {
      assert.equal(f.source, 'scalai-doc',
        `allow-same-origin on a ${f.source} iframe would let it read ScalAI`);
    }
  }
});
...
test('the doc scheme is registered standard and secure (gives its own origin)', () => {
  assert.match(main, /scheme:\s*['"]scalai-doc['"]/);
