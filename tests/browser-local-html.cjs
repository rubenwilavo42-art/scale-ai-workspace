// Run with Node. Two normal ScalAI processes use a disposable profile and local
...
if (!process.versions.electron) {
  ...
  app.whenReady().then(async () => {
    try {
      ...
      await until(() => run('!!document.querySelector(".browser-address") && !!window.scalai'), 'address bar');
      ...
      if (phase === 'write') {
        ...
        console.log('PASS: normal ScalAI restart restores the local HTML tab and selected profile; reload and a new tab work.');
      }
      app.quit();
