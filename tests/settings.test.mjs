...
import { readParamètres, writeParamètres, normalizeThème, themeBackground, THEMES, DEFAULT_THEME, normalizeView, VIEWS } from '../src/main/settings.js';
...
test('readParamètres returns {} for missing, empty, corrupt and non-object files', () => {
  const io = memIo();
  assert.deepEqual(readParamètres({ file: '/nope.json', io }), {});
  io.files.set('/a.json', '');
  assert.deepEqual(readParamètres({ file: '/a.json', io }), {});
  io.files.set('/b.json', '{ this is not json');
  assert.deepEqual(readParamètres({ file: '/b.json', io }), {});
  io.files.set('/c.json', '["an","array"]');
  assert.deepEqual(readParamètres({ file: '/c.json', io }), {});
});

test('writeParamètres merges rather than clobbering sibling keys', () => {
  const file = tmpFile();
  writeParamètres({ file, patch: { theme: 'operator' } });
  writeParamètres({ file, patch: { aiModel: { baseUrl: 'http://x/v1', model: 'm' } } });
  writeParamètres({ file, patch: { sttProvider: 'local', openaiKey: 'sk-test' } });
  // The theme write must have survived two later writers touching the same file.
  const doc = readParamètres({ file });
...
test('a null value deletes the key — how the UI clears an API key', () => {
  const file = tmpFile();
  writeParamètres({ file, patch: { openaiKey: 'sk-test', theme: 'paper' } });
  writeParamètres({ file, patch: { openaiKey: null } });
  const doc = readParamètres({ file });
...
test('a failing write reports {ok:false} instead of throwing', () => {
  ...
  const res = writeParamètres({ file: '/x.json', patch: { theme: 'paper' }, io });
...
test('creates the parent directory on first write', () => {
  ...
  const res = writeParamètres({ file, patch: { theme: 'paper' } });
  assert.equal(res.ok, true);
  assert.equal(readParamètres({ file }).theme, 'paper');
});
...
test('normalizeThème accepts all shipped themes', () => {
  for (const t of ['paper', 'operator', 'glass', 'graphite', 'soft', 'dusk']) assert.equal(normalizeThème(t), t);
});

test('normalizeThème falls back to the default for anything else', () => {
  ...
  for (const bad of ['neon', '', null, undefined, 42, 'GLASS']) assert.equal(normalizeThème(bad), DEFAULT_THEME);
});
...
test('themeBackground maps every theme to its first-paint color', () => {
  ...
  // unknown themes paint the default, matching normalizeThème
