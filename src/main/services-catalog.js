// What a ScalAI user can connect. Registry only: no IO here. Package names, env
...
// Guided services (Gmail, Drive) finish by writing the master. ScalAI delivers
// from there; the agent must not write each notebook itself, and must never
// write nami-browser into connections.json.
const GUIDED_FINISH = 'When it works, register it for this project by adding one entry to connections.json at the project root, under the standard "mcpServers" key (create the file if it is missing) — ScalAI copies it to every installed agent\'s own config from there.';
