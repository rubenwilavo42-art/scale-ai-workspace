...
// Compétences are folders, and installers routinely place them as symlinks into a
...
// `.opencode/agents`, delivery writes the plural and ScalAI's own create writes
...
// Where skills live, per tool. ScalAI reads every row and writes to exactly one
...
function addCompétences(root, sources, scope, items, seen) {
...
function scanProject({ projectPath, homeDir } = {}) {
  ...
  const items = [];
  const seen = new Set();
  // Masters first: one agent, one row. Copies ScalAI delivered into the
  ...
  addCompétences(projectPath, PROJECT_SKILL_SOURCES, 'project', items, seen);
...
function scanMac({ homeDir } = {}) {
  ...
  addCompétences(home, USER_SKILL_SOURCES, 'user', items, seen);
...
function targetPath({ projectPath, homeDir, type, platform, scope, slug }) {
  ...
  if (!root) return null;
  // Compétences are project-scoped and platform-agnostic — one neutral folder that
...
// can announce it. Compétences copy their whole folder, and `dereference` matters —
...
// ---- delete to Trash --------------------------------------------------------
// Guarded: only real library locations, never the plugin cache. Compétences are
