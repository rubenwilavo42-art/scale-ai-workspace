...
// No remote-debugging-port, global Target discovery, or ScalAI renderer target.
...
async function createCdpBridge({ entries, create, close, onCommand }) {
  ...
  const info = (e) => ({ targetId: e.targetId || e.id, browserContextId: 'scalai', type: 'page', title: e.view.webContents.getTitle(), url: e.view.webContents.getURL(), attached: true, canAccessOpener: false });
  ...
  async function command(method, params = {}, sid) {
    if (process.env.SCALAI_BROWSER_DEBUG) console.log('[browser-cdp]', method, sid || 'root');
    if (sid) {
      ...
      if (['DOM.setFileInputFichiers', 'Page.setDownloadBehavior'].includes(method)) throw new Error('File access is not enabled.');
    ...
    switch (method) {
      case 'Browser.getVersion': return { protocolVersion: '1.3', product: 'Chrome/' + process.versions.chrome, revision: '', userAgent: 'ScalAIBrowser' };
  ...
  server.on('connection', (ws) => {
    ...
    ws.on('message', async (raw) => {
      ...
      } catch (error) { if (process.env.SCALAI_BROWSER_DEBUG) console.log('[browser-cdp-error]', m?.method, error.message); if (m) send({ id: m.id, sessionId: m.sessionId, error: { code: -32000, message: error.message } }); }
    });
  });
