// ScalAI — Electron main process.
// Owns: the window, PTY terminal sessions,
// the open folder + its .claude scan, restart-proof state, and all IPC.

const { app, BrowserWindow, ipcMain: electronIpc, dialog, shell, clipboard, protocol, Menu, nativeImage, Notification } = require('electron');
...
const { runVersion, managerState } = require('./agent-manager');
const agentPermissions = require('./agent-permissions');
const agentFileOps = require('./agent-file-ops');
...
const { upsertMcpJson, upsertOpencode, removeService, detectServices, knownFichiers } = require('./mcp-config');
...
const { validServiceId: secureServiceId, validateEntry: validateMcpEntry, validateProjectPath: validateMcpProject, bundlePathAllowed } = require('./mcp-security');
...
const { writePointers, pointerStatus, linkNative, hasForeignCompétencesSection, POINTER_FILE } = require('./pointer.js');
...
const { shouldNotify, notificationCopy } = require('./notifications');
...
// scalai-doc:// — how a viewed HTML page and its neighbouring images are served.
//
...
// relative files while staying cross-origin to ScalAI's file:// renderer — it can
...
function installDocProtocol() {
  protocol.handle('scalai-doc', async (request) => {
...
// `productName: ScalAI` resolves the same packaged and unpackaged, so a dev run and the
// installed ScalAI.app would otherwise share one userData — the same state.json (recents,
...
// ScalAI-dev otherwise restore each other's desks into every screenshot. Review
// flags default to a disposable profile. An explicit --user-data must also be
// disposable for review: it is used as supplied and is never deleted by ScalAI.
...
const reviewProfile = createReviewProfile({
  ...
  reviewBuild: require('../../package.json').name === 'scalai-review',
});
...
// Sessions ScalAI is ending on purpose — quit, window close, tile close. pty.kill()
...
let state = { recentFolders: [], currentFolder: null, panels: [], panelsByFolder: {}, deskMetaByFolder: {}, windows: [] };
function loadState() {
  ...
  if (!state.deskMetaByFolder || typeof state.deskMetaByFolder !== 'object') state.deskMetaByFolder = {};
...
function readParamètres() { return settingsStore.readParamètres({ file: settingsFile() }); }
function writeParamètres(patch) { return settingsStore.writeParamètres({ file: settingsFile(), patch }); }
...
// Open Recent is the recents list, and View > Thème ticks the saved theme.
// Both change while the app runs, and a menu is a snapshot of the moment it was
// installed.
//
// Every ScalAI item sends a string to the window you are looking at. The focused
...
function refreshAppMenu(focusedWindow = BrowserWindow.getFocusedWindow() || win) {
  installAppMenu({
    ...
    theme: windowThèmes.get(focusedWindow?.webContents.id) || settingsStore.normalizeThème(readParamètres().theme),
...
// This window is ScalAI and nothing may replace it. Rendered content — a doc's
...
function createWindow(folder, bounds) {
  const w = new BrowserWindow({
    ...
    // tile head runs out of room even with its controls dropped. ScalAI is often a
    ...
    ...windowChrome(),
    backgroundColor: settingsStore.themeBackground(readParamètres().theme),
  ...
  wins.add(w); win = w;
  windowThèmes.set(wcId, settingsStore.normalizeThème(readParamètres().theme));
...
app.whenReady().then(() => {
  ...
  app.setAboutPanelOptions({
    applicationName: 'ScalAI',
    applicationVersion: app.getVersion(),
    copyright: 'Copyright © 2026 ScalAI',
    credits: 'Espace de travail pour agents IA, par ScalAI',
  });
  installDocProtocol();  // serve viewed HTML + its assets from scalai-doc://
  ...
  userPath({ settings: readParamètres() });
  ...
  try {
    ...
    setTimeout(() => engine.warm(stt.sttConfig(readParamètres(), process.env)), 1500);
  ...
  sendPing({
    settings: readParamètres(), saveParamètres: writeParamètres,
...
// for the process to actually go, and anyone reopening ScalAI inside that window
...
const browserViews = wireBrowserViews(browserIpc, { readParamètres, writeParamètres });
...
ipcMain.handle('usage:read', async (e) => {
  ...
  usagePending = (async () => {
    // Reuse ScalAI's detected binaries. A usage refresh must not launch a fresh
    // interactive login shell for every provider (rc scripts can hang).
    let timer;
    const envPath = await Promise.race([userPath({ settings: readParamètres() }), new Promise((resolve) => { timer = setTimeout(() => resolve(process.env.PATH || ''), 2000); })]);
    clearTimeout(timer);
    const agents = await detectAgents({ exec: (bin) => knownBin(bin) || findOnDisk(bin, { env: { ...process.env, PATH: envPath } }) });
    return require('./usage').readUsage({ agents, directory: path.join(app.getPath('userData'), 'usage'), envPath, settings: readParamètres() });
  })().finally(() => { usagePending = null; });
  return usagePending;
});

wireAcpLive(ipcMain, { readParamètres });
...
ipcMain.handle('boot', (e) => {
  ...
  return {
    ...
    demo: DEMO,
    review: REVIEW,
    theme: settingsStore.normalizeThème(readParamètres().theme),
    ...
    view: settingsStore.normalizeView(readParamètres().view),
    // --scene=<name> opens one surface on boot so it can be screenshotted; screenshots only
    scene: SCENE,
    // ask the provider registry, not the environment — a key typed into Paramètres
...
ipcMain.handle('desk-meta:save', (_e, { folder, meta }) => {
  const key = folderKey(folder);
  state.deskMetaByFolder[key] = (meta && typeof meta === 'object') ? {
    version: 1,
    ...
    view: meta.view === 'split' ? 'split' : 'desk',
    savedAt: Number.isFinite(meta.savedAt) ? meta.savedAt : Date.now(),
  } : { version: 1, activeIndex: null, expandedIndex: null, view: 'desk', savedAt: Date.now() };
  persist();
  return { ok: true };
});
ipcMain.handle('desk-meta:load', (_e, folder) => state.deskMetaByFolder[folderKey(folder)] || null);
...
function appUpdatedAt() {
  try {
    // .../ScalAI.app/Contents/MacOS/ScalAI → .../ScalAI.app
...
ipcMain.handle('update:status', async () => {
  ...
  // one field to begin with, and the pane duly announced "ScalAI 0.1.3, updated
...
// going. All the windows share one copy of ScalAI on disk, so they share one
...
ipcMain.handle('agents:status', (_e, { id } = {}) => agentStatus(id, { envKeys: storedEnvKeys(), settings: readParamètres() }));
ipcMain.handle('agents:version', async (_e, { id } = {}) => {
  const settings = readParamètres();
  const agents = await detectAgents({ settings });
  const agent = agents.find((a) => a.id === id);
  if (!agent || !agent.found) return { ok: false, version: '', error: 'Agent non installé.' };
  return runVersion({ ...agent, bin: agent.path || agent.bin }, { exec: (file, args, opts, cb) => execFile(file, args, { ...opts, env: buildChildEnv({ settings, purpose: 'agent', agentId: id }) }, cb) });
});
ipcMain.handle('agents:manager-state', async (_e, { id } = {}) => {
  const settings = readParamètres();
  const agents = await detectAgents({ settings });
  const agent = agents.find((a) => a.id === id);
  if (!agent) return { ok: false, error: 'Agent inconnu.' };
  const status = await agentStatus(id, { envKeys: storedEnvKeys(), settings });
  const version = agent.found ? await runVersion({ ...agent, bin: agent.path || agent.bin }, { exec: (file, args, opts, cb) => execFile(file, args, { ...opts, env: buildChildEnv({ settings, purpose: 'agent', agentId: id }) }, cb) }) : null;
  return { ok: true, state: managerState(agent, status, version) };
});
...
function claudeExec(argv) {
  return new Promise((resolve) => {
    ...
    execFile(bin, argv, { timeout: 20000, env: buildChildEnv({ settings: readParamètres(), purpose: 'agent', agentId: 'claude' }) }, (err) => {
      resolve(err ? { ok: false, error: redactChildError(err, { settings: readParamètres() }).split('\n')[0] } : { ok: true });
    });
  });
}
...
async function deliverConnexions({ scope, projectPath, agentIds }) {
...
ipcMain.handle('services:connectCustom', async (_e, { name, address, values, bundleDir, scope, agentIds, projectPath } = {}) => {
  try {
    ...
    let entry;
    if (bundleDir) {
      if (!bundlePathAllowed(bundleDir, os.homedir())) return { ok: false, error: 'MCP bundle must be installed inside ScalAI’s managed bundle directory.' };
    ...
    if (!secureServiceId(id)) return { ok: false, error: 'Invalid MCP connection name.' };
    const entryResult = validateMcpEntry(entry);
    if (!entryResult.ok) return entryResult;
...
ipcMain.handle('services:deliver', async (_e, { projectPath, agentIds } = {}) => {
  const out = [];
  out.push(...await deliverConnexions({ scope: 'user', projectPath, agentIds }));
  if (projectPath) out.push(...await deliverConnexions({ scope: 'project', projectPath, agentIds }));
  return out;
});
ipcMain.handle('services:disconnect', async (_e, { id, projectPath }) => {
  if (!secureServiceId(String(id || ''))) return { changed: [], error: 'Invalid MCP connection name.' };
  if (projectPath && !validateMcpProject(projectPath, 'project').ok) return { changed: [], error: 'Invalid MCP project folder.' };
  const home = os.homedir();
  const changed = [];
  ...
  const jsonFichiers = new Set(knownFichiers(projectPath, home).filter(([f]) => f.indexOf('.claude.json') < 0).map(([f]) => f));
  for (const scope of ['project', 'user']) {
    const targets = notebookTargets({ scope, projectPath, homeDir: home });
    for (const t of Object.values(targets)) if (t.kind === 'json' && t.file) jsonFichiers.add(t.file);
  }
  changed.push(...removeService({ files: [...jsonFichiers], id }));
  ...
  const viaCli = validServiceId(id) ? await new Promise((resolve) => {
    const bin = knownBin('claude') || 'claude';
    execFile(bin, ['mcp', 'remove', '--scope', 'user', id], { timeout: 20000, env: buildChildEnv({ settings: readParamètres(), purpose: 'agent', agentId: 'claude' }) }, (err) => resolve(!err));
  }) : false;
...
// Thème lives in settings.json so the window background matches on next launch.
ipcMain.on('theme:applied', (e, theme) => {
  if (!wins.has(BrowserWindow.fromWebContents(e.sender))) return;
  windowThèmes.set(e.sender.id, settingsStore.normalizeThème(theme));
  refreshAppMenu();
});
ipcMain.handle('theme:set', (_e, theme) => {
  if (REVIEW) return { ok: true };
  const saved = writeParamètres({ theme: settingsStore.normalizeThème(theme) });
  refreshAppMenu();
  return saved;
});

// Desk or Split, so the app reopens in the view you left.
ipcMain.handle('view:set', (_e, view) => REVIEW ? { ok: true } : writeParamètres({ view: settingsStore.normalizeView(view) }));

// The Paramètres page reads and writes settings.json directly. Only these keys are
...
ipcMain.handle('notifications:test', () => {
  const settings = readParamètres();
  if (settings.notificationsEnabled === false) return { ok: false, disabled: true };
  if (!Notification.isSupported()) return { ok: false, error: 'Les notifications système ne sont pas disponibles sur cet appareil.' };
  new Notification({ title: 'ScalAI', body: 'Les notifications sont activées.' }).show();
  return { ok: true };
});

ipcMain.handle('settings:get', () => {
  const s = readParamètres();
...
function storedEnvKeys() { const k = readParamètres().envKeys; return (k && typeof k === 'object' && !Array.isArray(k)) ? k : {}; }
...
ipcMain.handle('keys:set', (_e, { name, value }) => {
  ...
  if (!value || !String(value).trim()) return { ok: false, error: 'Collez d’abord le secret.' };
  const next = Object.assign({}, storedEnvKeys(), { [name]: String(value).trim() });
  const res = writeParamètres({ envKeys: next });
...
ipcMain.handle('settings:set', (_e, patch) => {
  ...
  const res = writeParamètres(clean);
...
ipcMain.handle('folder:make', async (e) => {
  const parent = BrowserWindow.fromWebContents(e.sender) || win;
  const res = await dialog.showEnregistrerDialog(parent, {
    title: 'Where should ScalAI work?',
    defaultPath: path.join(app.getPath('documents'), 'ScalAI'),
    buttonLabel: 'Create',
...
ipcMain.handle('pointer:status', (_e, args) => {
  ...
  try {
    const skills = projectCompétences(dir);
    const st = pointerStatus({ dir, skills, agentIds });
    const text = safeReadText(dir && path.join(dir, POINTER_FILE));
    return { ...st, skills: skills.map((s) => s.slug), foreignSection: hasForeignCompétencesSection(text) };
  } catch (e) { return { inSync: true, unlisted: [], stale: [], missingFichiers: [], listed: [], error: e.message }; }
});
...
function projectCompétences(dir) {
  if (!dir) return [];
...
ipcMain.handle('agent:fs', async (e, a) => {
  const input = { ...(a || {}), workspaceRoot: (a && a.workspaceRoot) || winFolders.get(e.sender.id) || state.currentFolder };
  return agentFileOps.execute(input, (payload) => sendWc(e.sender, 'permission:request', payload));
});
...
// why the ignore list runs before the debounce rather than after. Fermerd on
...
function sttEnv() { return { settings: readParamètres(), env: process.env }; }
...
function sessionEnv(path, launch, policy = sessionPolicy(launch)) {
  ...
  const env = Object.assign(buildChildEnv({ parentEnv: process.env, settings: readParamètres(), ...policy }), { TERM: 'xterm-256color', FORCE_COLOR: '1' });
  ...
  const theme = settingsStore.normalizeThème(readParamètres().theme);
...
ipcMain.handle('permissions:resolve', (_e, { id, decision }) => agentPermissions.resolvePermission(id, decision));
ipcMain.handle('permissions:clear-session', () => { agentPermissions.clearSessionPermissions(); return { ok: true }; });

ipcMain.handle('term:create', async (e, { id, cwd, workspaceRoot, cols, rows, kind, command, program, args, seed, cont, sid, acpSid, name, watchDone, oneShot, purpose, agentId }) => {
  const wc = e.sender;
  const effectiveWorkspace = workspaceRoot || winFolders.get(wc.id) || state.currentFolder;
  const workspaceCheck = agentPermissions.validateWorkspace({ workspaceRoot: effectiveWorkspace, cwd });
  if (!workspaceCheck.ok && (purpose === 'agent' || kind === 'run' || kind === 'harness' || kind === 'claude')) {
    sendWc(wc, 'permission:error', { id, error: workspaceCheck.error });
    return { ok: false, error: workspaceCheck.error };
  }
  if (purpose === 'agent' || kind === 'claude' || kind === 'harness') {
    const request = {
      workspaceRoot: effectiveWorkspace, cwd, agentId: agentId || (kind === 'claude' ? 'claude' : null),
      command: command || program || (kind === 'claude' ? 'claude' : ''), program, kind,
      scope: 'agent-run',
      credentialAccess: true, settings: readParamètres(),
    };
    const permission = await agentPermissions.requestPermission(request, (payload) => sendWc(wc, 'permission:request', payload));
    if (!permission.ok) {
      sendWc(wc, 'permission:denied', { id, error: permission.error });
      return { ok: false, denied: true, error: permission.error };
    }
    if (permission.decision === 'session') agentPermissions.rememberSession(request);
  }
  ...
  // second of launch.
  const envPath = await userPath({ settings: readParamètres() });
  ...
  if (kind === 'claude') {
    ...
    // watchDone marks a one-shot: a command ScalAI ran on the user's behalf and
    ...
    // after the spawn below. One-shots (watchDone) are ScalAI's errands, never
  ...
  } catch (err) { sendWc(wc, 'term:data', { id, data: '\r\n[could not start: ' + redactChildError(err, { settings: readParamètres() }) + ']\r\n' }); return { ok: false }; }
  ...
  p.onData((data) => {
    ...
    // title below, opposite direction: the shell talking to ScalAI.
  ...
  p.onExit(({ exitCode, signal }) => {
    ...
    // whether this teardown was ScalAI's own doing.
    ...
    try {
      const focused = BrowserWindow.fromWebContents(wc)?.isFocused?.() || false;
      const settings = readParamètres();
      if (shouldNotify({ enabled: settings.notificationsEnabled !== false, appFocused: focused, deliberate, code: exitCode, kind: 'session' }) && Notification.isSupported()) {
        const copy = notificationCopy({ title: discoverAgent || name || 'Agent', code: exitCode, note: exitNote({ code: exitCode, signal, deliberate }) });
...
ipcMain.handle('term:resize', (_e, { id, cols, rows }) => {
  if (process.env.SCALAI_PTY_LOG) {
    ptyResizeN++;
    const line = `${ptyResizeN} ${id} ${cols}x${rows}\n`;
    try { fs.appendFileSync(process.env.SCALAI_PTY_LOG, line); } catch (_) {}
