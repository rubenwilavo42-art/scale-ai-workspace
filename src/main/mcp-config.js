...
function knownFichiers(projectPath, home) {
  const out = [];
  if (projectPath) {
...
function detectServices({ projectPath, home, io = fsIo }) {
  const found = new Map();
  for (const [file, scope, platform, section] of knownFichiers(projectPath, home)) {
    const doc = readJson(file, io);
...
module.exports = { fsIo, readJson, upsertMcpJson, upsertOpencode, removeService, detectServices, knownFichiers };
