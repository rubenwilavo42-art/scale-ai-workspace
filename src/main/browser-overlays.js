  ...
  ipcMain.handle('browser:overlays', async (event, payload = {}) => {
    ...
    if (!records) {
      ...
      w.on('close', async event => {
        ...
        try { const result=await dialog.showMessageBox(w,{type:'question',message:'Discard pending browser annotations and close ScalAI?',detail:'These temporary notes have not been inserted into a session.',buttons:['Keep notes','Discard and close'],defaultId:0,cancelId:0});
