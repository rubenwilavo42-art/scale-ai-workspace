...
contextBridge.exposeInMainWorld('scalai', {
  ...
  browserFermer: (id) => ipcRenderer.invoke('browser:close', { id, confirmed:true }),
  ...
  saveDeskMeta: (args) => ipcRenderer.invoke('desk-meta:save', args),
  loadDeskMeta: (folder) => ipcRenderer.invoke('desk-meta:load', folder),
  ...
  // Finder opened a file with ScalAI. adopt says the folder has to change first;
  ...
  // One channel for the whole menu bar. Every ScalAI item in it is a string this
  ...
  agentVersion: (id) => ipcRenderer.invoke('agents:version', { id }),
  agentManagerState: (id) => ipcRenderer.invoke('agents:manager-state', { id }),
  ...
  keysSupprimer: (name) => ipcRenderer.invoke('keys:delete', { name }),
  ...
  librarySupprimer: (args) => ipcRenderer.invoke('library:delete', args),
  ...
  // Fichiers dragged in from Finder. Paths come from droppedFilePath above, never
  ...
  agentFileOperation: (args) => ipcRenderer.invoke('agent:fs', args),
  ...
  resolvePermission: (id, decision) => ipcRenderer.invoke('permissions:resolve', { id, decision }),
  clearPermissionSession: () => ipcRenderer.invoke('permissions:clear-session'),
  onPermissionRequest: (cb) => { const h = (_e, ev) => cb(ev); ipcRenderer.on('permission:request', h); return () => ipcRenderer.removeListener('permission:request', h); },
  onPermissionDenied: (cb) => { const h = (_e, ev) => cb(ev); ipcRenderer.on('permission:denied', h); return () => ipcRenderer.removeListener('permission:denied', h); },
  onPermissionError: (cb) => { const h = (_e, ev) => cb(ev); ipcRenderer.on('permission:error', h); return () => ipcRenderer.removeListener('permission:error', h); },
  ...
  // A newer ScalAI exists — { version, url }. Only ever fires when there is one;
