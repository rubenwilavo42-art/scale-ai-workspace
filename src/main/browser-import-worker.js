...
async function readImportSource({source,categories,directory,cancelled,signal,send,passwordFor=keychainPassword,batch}) {
  ...
  for(const category of ['cookies','passwords','history']) {
    ...
    try {
      ...
      send({type:'result',category,error:locked?'Source database is locked. Fermer the source browser and retry.':'Could not read the source '+category+' database. Check file access and retry.'});
