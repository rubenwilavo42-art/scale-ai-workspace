...
function claudeSkippedLocal(home) {
  return claudeFichiers(home).some((file) => readJson(file) === SKIPPED);
}
function claudeRows(home, directory, now) {
  let rows = [];
  for (const file of claudeFichiers(home)) { const data = usable(readJson(file)); if (data) rows = mergeReports(rows, claudeUsage(data, now)); }
