// "Did the command ScalAI typed into that shell finish, and how did it go?"
//
...
// shell, not ScalAI, owns the command: pty exit only fires when the *shell*
...
//
//   curl … | bash; printf '\033]1337;ScalAIRunDone=%s\007' "$?"
//
...
// channel ScalAI already reads claude's session titles from (osc-title.js), used
// the other way round.
//
...
const OPEN = ']1337;ScalAIRunDone=';
const DONE_RE = /\]1337;ScalAIRunDone=(-?\d{1,5})(?:|\\)/;
...
function doneSuffix(command) {
  return `${command}; printf '\\033]1337;ScalAIRunDone=%s\\007' "$?"`;
}

// The suffix cannot be TYPED into an interactive shell, which is how a run tile
// used to be driven: a pty echoes its input, so the user watched ScalAI's own
