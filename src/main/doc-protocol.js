// The path rules for the scalai-doc:// scheme, kept pure so they can be tested
...
// and it still cannot reach ScalAI (see main.js and the design note). This file
...
// A nami-doc URL is  scalai-doc://doc/<root>/<rel>  where both parts are
...
function buildDocUrl(root, target) {
  ...
  return 'scalai-doc://doc/' + encodeURIComponent(root) + '/' + rel.split(path.sep).map(encodeURIComponent).join('/');
}
...
function parseDocUrl(urlString) {
  ...
  if (u.protocol !== 'scalai-doc:' || u.host !== 'doc') return null;
