...
// what you paste into ScalAI is what gets used.
...
function status({ settings = {}, env = {}, deps } = {}) {
  ...
  const providers = PROVIDERS.map((p) => {
    ...
    return {
      ...
      // true only when the user saved a key in ScalAI — the UI shows it masked
      keyEnregistrerd: !!(p.needsKey && (shared[p.keyEnv] || settings[p.needsKey])),
