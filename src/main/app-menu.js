...
//
// ScalAI shipped with Electron's stock menu, which is why the ScalAI menu offered
// Services, Hide Others and Show All, why Edit offered smart quotes to an app
// whose text is prompts and paths, and why About ScalAI opened the grey macOS
...
// the configured ScalAI site. External destinations stay configurable so a
// build never pretends to know a repository or website that has not been configured.
const LINKS = {
  repo: REPO,
...
// theme:graphite, open-recent:/Users/x/project.
const COMMANDS = [
...
const THEMES = [['glass', 'Clair'], ['dusk', 'Sombre']];
...
function buildMenuTemplate({
  open, send, newWindow, platform = process.platform, name = 'ScalAI', theme = 'paper', recents = [],
} = {}) {
  const mac = platform === 'darwin';
  const cmd = (label, command, extra = {}) => ({ label, ...extra, click: () => send(command) });
  const link = (label, url) => url ? ({ label, click: () => open(url) }) : null;

  // Two of these are the whole reason the branch exists. About ScalAI carries no
  // role, because `role: 'about'` is the stock panel; it opens ScalAI's own About
  // pane instead, which already knows the version, the day this copy landed,
  // whether a newer one exists and the licence. Paramètres takes the slot
  // Services used to hold, and ⌘, already did this with no menu to say so.
  const appSubmenu = [
    cmd('About ScalAI', 'about'),
    cmd('Check for Updates', 'update-check'),
    SEP,
    cmd('Paramètres', 'settings', { accelerator: 'CommandOrControl+,' }),
    SEP,
    { role: 'hide' },
    SEP,
    { role: 'quit' },
  ...
  // which binds ⌘W to Fermer Window, and ScalAI binds ⌘W to close the active
  ...
  // window and lose every session in it". Fermer Pane is routed to the same
  ...
  const fileSubmenu = [
    ...
    SEP,
    cmd('Enregistrer', 'save', { accelerator: 'CommandOrControl+S' }),
    cmd('Reveal in Finder', 'reveal'),
    SEP,
    cmd('Fermer Pane', 'close-pane', { accelerator: 'CommandOrControl+W' }),
    // Off macOS there is no app submenu, so File is the only place these can go.
    ...(mac ? [] : [SEP, cmd('Paramètres', 'settings', { accelerator: 'CommandOrControl+,' }), { role: 'quit' }]),
  ];
  ...
  const editSubmenu = [
    ...
    cmd('Voice Paramètres', 'settings:voice'),
  ];
  ...
  // to append the list of open windows underneath. ScalAI is a window per project
  ...
  // already in Paramètres.
  //
  // ★ Star ScalAI lives here and not in the ScalAI menu on purpose. The ScalAI menu
  // is opened with intent, for Paramètres or Quit, and an ask parked there taxes
  // every one of those visits. Help is where a Mac user looks for the project
  // itself, and About ScalAI already reaches the star in one hop: the pane it
  // opens has the star button as its primary action.
  const helpSubmenu = [
    link('ScalAI Docs', LINKS.docs),
    cmd('Keyboard Shortcuts', 'settings:shortcuts'),
    SEP,
    link('ScalAI on GitHub', LINKS.repo),
    link('★ Star ScalAI', LINKS.repo),
...
// Every item in the tree, at any depth. Submenus are three deep now (Thème,
...
function installAppMenu({ Menu, shell, app: electronApp, send, newWindow, theme, recents }) {
  const template = buildMenuTemplate({
    ...
    send,
    newWindow,
    name: (electronApp && electronApp.name) || 'ScalAI',
    theme,
    recents,
  });
