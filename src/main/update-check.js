// Is there a newer ScalAI than the one running?
//
...
const LATEST = process.env.SCALAI_UPDATE_URL || null;

async function fetchLatest(url = LATEST) {
  if (!url) throw new Error('ScalAI update feed is not configured');
  const res = await fetch(url, {
    headers: { Accept: 'application/vnd.github+json', 'User-Agent': 'ScalAI' },
