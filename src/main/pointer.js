...
const { KNOWN_AGENTS, POINTER_FILE, contextFichiersFor } = require('./agents-detect.js');

const START = '<!-- nami:skills start -->';
const END = '<!-- nami:skills end -->';

// Three lines, and they buy something specific: ScalAI never has to be right about
...
function findMarkers(text) {
  ...
  if (starts.length > 1) throw new Error('AGENTS.md has more than one ScalAI skills block — remove the extra one and try again.');
  if (!starts.length) return null;
  const end = text.indexOf(END, starts[0]);
  if (end === -1) throw new Error('AGENTS.md has an unclosed ScalAI skills marker — restore or remove it and try again.');
...
// A `## Compétences` heading the user wrote themselves. Worth knowing about — two
// Compétences sections in one file is confusing — but never worth rewriting: their
// wording is usually better than anything generated from frontmatter.
function hasForeignCompétencesSection(text) {
  const src = String(text == null ? '' : text);
  let found = null;
  try { found = findMarkers(src); } catch (_) { found = null; }
  const outside = found ? src.slice(0, found.start) + src.slice(found.end) : src;
  return /^#{1,6}\s+(?:skills|compétences)\b/im.test(outside);
}
...
function writePointers({ dir, skills, agentIds, dryRun } = {}) {
  ...
  const files = contextFichiersFor(agentIds);
...
function pointerStatus({ dir, skills, agentIds } = {}) {
  const blank = { inSync: true, unlisted: [], stale: [], missingFichiers: [], listed: [], error: '' };
  if (!dir) return blank;
  const have = (skills || []).filter((s) => s && s.slug).map((s) => s.slug);
  const files = contextFichiersFor(agentIds);
  const missingFichiers = files.filter((f) => !fs.existsSync(path.join(dir, f)));
  let listed = [];
  let error = '';
  ...
  if (!have.length && !listed.length) return { ...blank, missingFichiers: [] };
  const unlisted = have.filter((s) => !listed.includes(s));
  const stale = listed.filter((s) => !have.includes(s));
  return {
    inSync: !unlisted.length && !stale.length && !missingFichiers.length && !error,
    unlisted, stale, missingFichiers, listed, error,
  };
}
...
function linkNative({ dir, slugs, agentIds } = {}) {
  ...
  try {
    for (const agent of KNOWN_AGENTS) {
      if (!agent.projectCompétencesDir || !ids.has(agent.id)) continue;
      const nativeDir = path.join(dir, agent.projectCompétencesDir);
      const depth = agent.projectCompétencesDir.split('/').filter(Boolean).length;
      ...
      for (const slug of want) {
        ...
        linked.push(path.join(agent.projectCompétencesDir, slug));
      }
      ...
      for (const name of safeList(nativeDir)) {
        ...
        swept.push(path.join(agent.projectCompétencesDir, name));
      }
    }
...
module.exports = {
  ...
  renderBlock, spliceBlock, readBlock, hasForeignCompétencesSection,
