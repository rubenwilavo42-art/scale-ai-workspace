...
// The renderer can ask for a local page to leave ScalAI's sandbox, so this gate
...
function resolveBrowserInput(value, { homePath = os.homedir() } = {}) {
  ...
  if (/^["'](?:\/|~\/|file:)/i.test(text)) {
    if (text.at(-1) !== text[0]) throw new Error('Fermer the quote around the file path.');
    text = text.slice(1, -1);
  }
  ...
  try {
    ...
    if (['EACCES', 'EPERM'].includes(error.code)) throw new Error('ScalAI cannot read this HTML file. Check its file permissions.');
    throw error;
  }
