...
const MARKER_HEAD = 'made by ScalAI from agents/';
...
// `tool` (singular) is ScalAI's own: which tool this agent was written for. It is
...
function deliveryState({ projectPath, slug, agentIds, io = fsIo, homeDir }) {
  ...
  return (agentIds || []).map((agent) => {
    ...
    if (!t) return { agent, slug, state: 'none', reason: `ScalAI has no agent format for ${agent}` };
