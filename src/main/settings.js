...
function readParamètres({ file, io = fsIo }) {
...
function writeParamètres({ file, patch, io = fsIo }) {
  try {
    const doc = readParamètres({ file, io });
...
// somebody who has never seen ScalAI before. Anyone who has chosen a theme keeps
...
function normalizeThème(name) { return THEMES.includes(name) ? name : DEFAULT_THEME; }
...
function themeBackground(name) { return THEME_BG[normalizeThème(name)]; }

module.exports = { readParamètres, writeParamètres, fsIo, normalizeThème, themeBackground, THEMES, DEFAULT_THEME, normalizeView, VIEWS };
