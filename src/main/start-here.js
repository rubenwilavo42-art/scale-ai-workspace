// The note ScalAI leaves in a folder it made for you.
//
...
// the file tree, it opens in ScalAI's own viewer, and the newcomer's first ask can
...
This is **${name}**, the folder you just gave ScalAI.

ScalAI can read and change things in here, and nowhere else on your Mac. That is
the whole safety model, and it is why every session asks you for a folder first.

## ScalAI runs agents. It isn't one.

ScalAI is the desk. The thinking is done by an agent: Claude Code, Codex and a
few others. Each one signs in with **your own account**, so there is no ScalAI
account and no second bill. If you have none installed yet, press
**Nouvelle session** and pick one from the list; it installs from there.
...
You can delete this file whenever you like. It is yours now, and ScalAI will not
put it back.
`;
}
