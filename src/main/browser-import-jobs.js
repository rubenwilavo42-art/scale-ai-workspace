...
function createImportJobs({ resolveSource, resolveProfile, createWorker, applyBatch, onChange = () => {} }) {
  ...
  function start(owner,args) {
    if(closing) throw Error('ScalAI is closing.');
    if(typeof args.profileId!=='string' || !args.profileId) throw Error('Choose a destination ScalAI profile before importing.');
      ...
      worker.on('message',message=>{
        ...
        if(message.type==='stage' && j.state!=='cancelling') {
          ...
          j.chain=j.chain.then(async()=>{
            ...
            if(r.failed && !j.results[category].reasons.includes('Some rows were refused by ScalAI.')) j.results[category].reasons.push('Some rows were refused by ScalAI.');
            emit(j);
