// Streamable HTTP MCP, with a distinct bearer URL per ScalAI session. Browser
...
const OBSERVE_TOOLS = new Set(['browser_snapshot', 'browser_navigate', 'browser_navigate_back', 'browser_wait_for', 'browser_tabs', 'browser_console_messages', 'browser_network_requests']);
const MESSAGE_TOOLS = [
  { name: 'nami_sessions', description: 'List sessions you may message on this ScalAI desk.', inputSchema: { type: 'object', properties: {} } },
...
const SCALAI_TOOLS = [
  { name: 'nami_browser_screenshot', description: 'Read a screenshot of an exact shared ScalAI Browser tab. Returns the visible page image, title, URL and access time. Never uses an external browser.', inputSchema: { type: 'object', properties: { tabId: { type: 'string' } }, required: ['tabId'] } },
  { name: 'nami_browser_tabs', description: 'List the exact ScalAI Browser tabs currently shared with this session. Use these tab IDs for screenshots.', inputSchema: { type: 'object', properties: {} } },
  { name: 'nami_read_annotation_image', description: 'Read an explicitly inserted ScalAI annotation image by its opaque ID. No arbitrary files.', inputSchema: { type: 'object', properties: { id: { type: 'string' } }, required: ['id'] } },
  { name: 'nami_read_session_context', description: 'Read the latest published visible context of an explicitly linked ScalAI session. Terminal snapshots can be incomplete. Does not send a message or start a turn.', inputSchema: { type: 'object', properties: { sourceId: { type: 'string' } }, required: ['sourceId'] } },
];
async function createBrowserMcp({ access, views, create, remove, send, notifyMessage, contexts, images, onActivity }) {
  ...
  let schemaPromise;
  async function toolSchema(mode = 'interact') {
    if (!schemaPromise) schemaPromise = (async () => {
      ...
      const server = await createConnection({}, async () => { throw new Error('No page is needed to list ScalAI Browser tools.'); });
      const client = new Client({ name: 'nami-schema', version: '1.0.0' });
      const [a, b] = InMemoryTransport.createLinkedPair();
      try { await server.connect(a); await client.connect(b);
        return (await client.listTools()).tools.filter(t => ALLOWED_TOOLS.has(t.name) && (mode === 'observe' ? OBSERVE_TOOLS.has(t.name) : true)).map(tool => {
          const inputSchema = structuredClone(tool.inputSchema);
          if (inputSchema.properties) delete inputSchema.properties.filename;
          return { ...tool, description: 'ScalAI Browser only. ' + tool.description, inputSchema };
        });
  ...
  async function engine(route) {
    ...
    route.engine = (async () => {
      ...
      const bridge = await createCdpBridge({ entries, create: async (url) => {
        const first = entries().find(e=>!e.record?.local); if (!first) throw new Error('Open the website in ScalAI and grant its browser tab access first. Local HTML permission does not include signed-in browser profiles.');
      ...
      const client = new Client({ name: 'scalai', version: '1.0.0' });
      route.client = client;
  ...
  async function dispatch(route, message) {
    ...
    if (message.method === 'tools/list') {
      const mode = s.browserMode === 'observe' ? 'observe' : 'interact';
      return { tools: [...await toolSchema(mode), ...SCALAI_TOOLS, ...MESSAGE_TOOLS] };
    }
    if (message.method !== 'tools/call') throw new Error('Unsupported MCP method.');
    const { name, arguments: args = {} } = message.params || {};
    if (route.updating || route.revoked) throw new Error('ScalAI Browser access is being updated. Try again.');
    if (s.browserMode === 'observe' && !OBSERVE_TOOLS.has(name) && !name.startsWith('nami_')) throw new Error('This session has read-only browser access. Ask for interaction access to modify pages.');
    ...
    if (name === 'nami_browser_screenshot') {
      ...
      if (!e || !access.allows(route.id, e.id)) throw new Error('This ScalAI Browser tab is not shared with the session.');
      ...
      catch (_) { const message = 'The browser image is unavailable. Reveal the tab in ScalAI and try again.'; activity(route, e.id, name, message); throw new Error(message); }
    ...
    if ((name==='browser_navigate'||(name==='browser_tabs'&&args.action==='new')) && [...s.views].every(id=>views.get(id)?.record?.local)) throw new Error('Open the website in ScalAI and grant its browser tab access first. Local HTML permission does not include signed-in browser profiles.');
    ...
    if (!ALLOWED_TOOLS.has(name)) throw new Error('Tool is not available in ScalAI.');
