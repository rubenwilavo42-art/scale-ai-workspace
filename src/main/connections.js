...
//
// The master file IS the standard `mcpServers` shape, not a ScalAI format:
...
// ScalAI. Only OpenCode (its own JSON dialect) and Codex (TOML) need translating;
...
function upsertMaster({ scope, projectPath, homeDir, id, entry, io = fsIo }) {
  if (reservedServiceId(id)) return { ok: false, error: 'ScalAI Browser is not a catalog connection.' };
  const file = masterPath({ scope, projectPath, homeDir });
