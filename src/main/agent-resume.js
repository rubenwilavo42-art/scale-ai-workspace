...
// pins --session-id at spawn, so a restore is --resume of an id ScalAI already
...
// A run tile ScalAI spawned as an agent carries exactly the bare bin; anything
...
//   opencode  session.title — placeholder "Nouvelle session - <iso date>" until the
...
function readSessionTitle(agent, cwd, sid, home = os.homedir()) {
  try {
    ...
    if (agent === 'opencode') {
      ...
      return t && !/^Nouvelle session - /.test(t) ? label(t) : null;
    }
    if (agent === 'hermes') {
