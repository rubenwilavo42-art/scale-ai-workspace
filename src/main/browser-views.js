...
function namiThèmeIsDark(settings) {
  const theme = settings?.theme || '';
  return theme === 'operator' || theme === 'graphite' || theme === 'dusk';
}
// "System" means ScalAI's own theme. It used to fall through to the Mac's dark
// mode when ScalAI was light, which on a light desk over a dark Mac gave a dark
// new tab, then a dark website, then a light desk again — the flicker people
// reported. ScalAI is the system the tab lives in.
function blankIsDark(mode, settings) {
  if (mode === 'dark') return true;
  if (mode === 'light') return false;
  return namiThèmeIsDark(settings);
}
...
// the desk around it. ScalAI's own window styles itself by data-theme and never
// consults this, so nothing there moves.
function syncNativeThème(settings) {
  try { require('electron').nativeThème.themeSource = blankIsDark(blankMode(settings), settings) ? 'dark' : 'light'; } catch {}
}
...
function wireBrowserViews(ipcMain, { readParamètres, writeParamètres }) {
  ...
  let browserEnabled = !!readParamètres().browserEnabled;
  ...
  try {
    require('electron').nativeThème.on('updated', () => {
      if (blankMode(readParamètres()) === 'system') applyBlankAppearance('system');
    });
  } catch {}
  syncNativeThème(readParamètres());
  ...
  const mainWindow = (event) => {
    ...
    if (!w || event.sender !== w.webContents || event.senderFrame !== w.webContents.mainFrame) throw new Error('Browser action is not from ScalAI.');
    return w;
  };
  ...
  function getPartition(w, profileId = 'default', localId = null) {
    ...
    record.session.on('will-download', (_event, item, wc) => {
      ...
      if (mode === 'auto') item.setEnregistrerPath(uniqueDownloadPath(app.getPath('downloads'), item.getFilename()));
    });
    record.session.protocol.handle('scalai-doc', async (request) => {
  ...
  async function create(w, args, { pendingCount = 0, replacing = null, rememberProfile = false } = {}) {
    ...
    try {
      ...
      if (replacing) {
        ...
        if (w.isDestroyed()) throw new Error('The ScalAI window has closed.');
      ...
      if (record.local) {
        record.roots.clear(); record.session.protocol.unhandle('scalai-doc'); partitions.delete(record.key);
  ...
  async function remove(id, { notify = true, confirmed = false } = {}) {
    ...
    if (e.pendingCount && !confirmed) throw new Error('This tab has pending annotations. Review or discard them in ScalAI before closing the tab.');
    views.delete(id);
    ...
    if (e.record.local) { e.record.roots.clear(); e.record.session.protocol.unhandle('scalai-doc'); partitions.delete(e.record.key); await e.record.session.clearStorageData(); }
  }
  const guarded = (channel, action) => ipcMain.handle(channel, async (ev, args = {}) => {
    try {
      ...
      const run = mutations.then(() => {
        if (w.isDestroyed() || ev.sender.isDestroyed()) throw new Error('The ScalAI window has closed.');
  ...
  guarded('browser:action', async (w, { id, action, url, value }) => {
    ...
    if (action === 'navigate') {
      ...
      const result = await dialog.showEnregistrerDialog(w, { title: 'Enregistrer browser screenshot', defaultPath: 'nami-browser.png', filters: [{ name: 'PNG image', extensions: ['png'] }] });
  ...
  async function startImport(w, args) {
    if (typeof args.profileId !== 'string' || !args.profileId) throw new Error('Choose a destination ScalAI profile before importing.');
  ...
  guarded('browser:profiles', async (w, args) => {
    ...
    if (importing && (typeof args.profileId !== 'string' || !args.profileId)) throw new Error('Choose a destination ScalAI profile before importing.');
    const profileId = args.profileId || profiles.defaultId();
    ...
    if (action === 'set-default') {
      ...
      if (args.confirmed !== true) throw new Error('Confirm clearing this ScalAI browser data first.');
      ...
      await mutateProfile(profileId, async () => {
        ...
        if (action === 'remove' || args.siteData) {
          ...
          // Fermer documents before clearing so running scripts cannot repopulate
        ...
        if (action === 'remove') { profiles.remove(profileId); record.session.protocol.unhandle('scalai-doc'); partitions.delete(profileId); }
      });
      } finally { importBlocks.delete(profileId); }
    } else if (action === 'import-passwords') {
      if (importJobs.active(profileId)) throw new Error('Annuler the running browser import before importing a password CSV.');
      profiles.get(profileId);
      ...
      writeParamètres({ browserNewTab: mode });
      syncNativeThème(readParamètres());
    ...
    return { ...output, profiles: profiles.list(), defaultProfileId: profiles.defaultId(), importJobs: importJobs.list(w.webContents.id), capabilities: { passwordCsv: typeof safeStorage.encryptString === 'function', cookieImport: cookieImportStatus(), cookies: true, history: true, newTab: blankMode(readParamètres()) } };
  });
  ...
  guarded('browser:enable', async (_w, { enabled }) => {
    const result = writeParamètres({ browserEnabled: !!enabled });
  ...
  guarded('browser:grant', async (w, { id, viewIds = [], peers = [], sourceIds, expectedIdentities, expectedSourceIdentities, mode = 'interact' }) => {
    if (!browserEnabled) throw new Error('Enable the browser connection in Paramètres first.');
    if (!['observe', 'interact'].includes(mode)) throw new Error('Invalid browser access mode.');
    ...
    await service.refresh(id, () => {
      ...
      access.grant(id, w.webContents.id, viewIds);
      const session = access.get(id);
      session.peers = peers.filter(p => p !== id);
      session.browserMode = mode;
    });
    return service.connection(id);
  });
