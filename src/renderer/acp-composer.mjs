...
//   onSend(text, attachments)   onPickFichiers()   onCommand(name)  — intercepts
//   onModeCycle()               onModelPick()
// State setters returned: setCommands, setCompétences, setMode, setModel, setUsage,
...
export function createComposer(host, o) {
  ...
  host.innerHTML = `
    ...
    <div class="cw-inrow">
      <button class="cw-plus" title="Fichiers, commands and skills">＋</button>
  ...
  function openMenu(filter) {
    ...
    if (skRows.length) html += '<div class="hd">Compétences</div>' +
    ...
    if (filesRow) filesRow.onclick = (e) => { e.stopPropagation(); popEl.hidden = true; if (o.onPickFichiers) o.onPickFichiers(); else fileInput.click(); };
  ...
  return {
    ...
    setCompétences: (list) => { skills = list || []; },
