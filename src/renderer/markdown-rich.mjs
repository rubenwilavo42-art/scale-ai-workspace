...
export async function mountMarkdownEditor(root, markdown, options) {
  const { createScalAIMarkdownEditor } = await loadMarkdownEditor();
  return createScalAIMarkdownEditor(root, markdown, options);
}
...
export function markdownImageUrl(documentPath, source) {
  ...
  return 'scalai-doc://doc/' + encodeURIComponent(dir) + '/' + src.split('/').map(encodePathPart).join('/');
}
