...
// session, which is a pty. dropFichiersOnPanel has quoted unconditionally since it
...
// A viewed HTML file → its scalai-doc:// URL, served from its own folder as root so
// its relative images resolve while the page stays cross-origin to ScalAI. Mirrors
...
export function docUrl(absPath) {
  ...
  return 'scalai-doc://doc/' + encodeURIComponent(root) + '/' + encodeURIComponent(rel);
}
