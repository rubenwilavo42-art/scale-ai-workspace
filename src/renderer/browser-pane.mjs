...
import { browserParamètresHtml, wireBrowserParamètres } from './browser-settings.mjs';
...
export function createBrowserPane({ api, state, tiles, uid, esc, helpIcon, isFile, isSession, pin, focus, refresh, save,
  ...
  const importStage = j => ({preparing:'Preparing source',keychain:'Waiting for Keychain',reading:'Reading selected data',copying:'Copying into '+j.profileName,cancelling:'Annulerling import',complete:'Import complete',partial:'Import partially completed',cancelled:'Import cancelled',failed:'Import failed'}[j.stage]||'Importing');
  ...
  function tabs(p, rec) {
    ...
    rec.companionTabs.innerHTML = siblings.map(x=>`<span class="companion-tab-item${x.id===p.id?' selected':''}"><button class="companion-tab" data-view-id="${esc(x.id)}" title="${esc(x.title)}" aria-pressed="${x.id===p.id}">${panelIcon?.(x)||''}<span>${esc(x.title)}</span></button><button class="companion-close" data-close-id="${esc(x.id)}" aria-label="Fermer ${esc(x.title)}" title="Fermer tab">×</button></span>`).join('')+'<button class="companion-add" title="Nouvel onglet du navigateur" aria-label="Nouvel onglet du navigateur">＋</button>';
  ...
  function mount(p, rec) {
    ...
    rec.body.innerHTML = `<form class="browser-address">${button('back', 'Back', 'back')}${button('forward', 'Forward', 'forward')}${button('refresh', 'Reload', 'reload')}<input aria-label="Adresse du navigateur" placeholder="Rechercher ou saisir une adresse" value="${esc(p.filePath || (p.url && p.url !== 'about:blank' ? p.url : ''))}" spellcheck="false"><button type="button" class="browser-annotate" aria-pressed="false" title="Annotate">Annotate</button>${button('more', 'Browser menu', 'menu')}</form><div class="browser-error" role="status" hidden></div><button type="button" class="browser-import-status" hidden></button><div class="browser-viewport"></div><div class="browser-selection" hidden><button class="btn btn--small">Selection · Add to session…</button></div>`;
    ...
    rec.disposeBrowser = () => { disposeAnnotations(); ro.disconnect(); api.browserFermer(p.id).catch(() => {}); };
  ...
  function openMenu(p, anchor) {
    ...
    const actions = [
      ['Ouvrir dans Chrome \u2197', () => openOutside(p)],
  ...
  function findInPage(p) {
    ...
    bar.innerHTML='<input aria-label="Find in page" placeholder="Find in page"><button class="btn btn--small">Find</button><button class="btn btn--small" type="button" aria-label="Fermer find">×</button>';
  ...
  async function renderProfiles() {
    ...
    const modal=dialog('modal modal--browser', '<div class="modal-head"><span class="title">Browser profiles</span></div><div class="modal-body browser-profile-body">Chargement…</div><div class="modal-foot"><button class="btn btn--go browser-profile-done" id="profiles-done">Done</button></div>');
    ...
    (async()=>{
      ...
      contents.textContent=[n(c.cookies,'cookie','cookies')+(c.session?' ('+n(c.session,'session cookie','session cookies')+')':''),
        c.passwords===null?'Enregistrerd passwords: open to view':n(c.passwords,'password','passwords'), n(c.history,'history row','history rows')].join(' \u00b7 ');
    })();
    ...
    q('#profile-import',host).onclick=()=>ask('Import a password CSV into '+chosen.name+'?',async()=>{const out=await run({action:'import-passwords'});if(out)result.textContent=out.canceled?'Annulerled.':out.message||('Imported '+(out.imported??0)+' passwords.');});
    q('#profile-clear',host).onclick=()=>{const siteData=q('#clear-signins',host).checked,credentials=q('#clear-passwords',host).checked;if(!siteData&&!credentials){result.textContent='Choisissez les données à effacer.';return;}ask('Clear selected data from '+chosen.name+'? Any import into this profile will be cancelled.',async()=>{if(await run({action:'clear',siteData,credentials,confirmed:true})){result.textContent='Cleared.';}});};
    ...
    q('#profile-passwords',host).ontoggle=async e=>{
      ...
      if(!saved){q('#profile-credentials',host).textContent='Could not unlock saved passwords. Fermer and reopen this section to try again.';return;}
      q('#profile-credentials',host).innerHTML=(saved.credentials||[]).map(c=>`<div class="browser-credential"><span>${esc(c.origin)}<small>${esc(c.username)}</small></span>${canFill&&c.origin===currentOrigin?`<button class="btn btn--small" data-fill="${esc(c.id)}">Fill</button>`:''}<button class="btn btn--small" data-delete="${esc(c.id)}">Supprimer</button></div>`).join('')||'<p class="note">No saved passwords.</p>';
      host.querySelectorAll('[data-delete]').forEach(b=>b.onclick=()=>ask('Supprimer this saved password from ScalAI?',async()=>{if(await run({action:'delete-credential',credentialId:b.dataset.delete}))show({...o});}));
  ...
  async function renderImportJob(o) {
    const modal=dialog('modal modal--browser', `<div class="modal-head"><span class="title">Browser import</span></div>
      ...
      <div class="modal-foot"><button class="btn" id="import-cancel">Fermer</button><button class="btn" id="import-stop">Annuler import</button><button class="btn btn--go browser-import-go" id="import-go" hidden disabled>Import again</button></div>`);
    const current=()=>state.overlay===o && modal.isConnected;
    q('#import-cancel',modal).onclick=close;
    let job;
    const update=j=>{
      if(!current() || j.id!==o.jobId)return; job=j;
      q('.browser-import-context',modal).textContent=j.sourceName+' → ScalAI · '+j.profileName;
      ...
      q('.browser-import-results',modal).innerHTML=Object.entries(j.results).map(([category,r])=>`<div class="browser-import-result-row"><strong>${esc({cookies:'Cookies',passwords:'Enregistrerd passwords',history:'Browsing history'}[category])}</strong><span>${r.copied} copied · ${r.skipped} skipped · ${r.failed} failed</span>${r.error?`<p>${esc(r.error)}</p>`:''}${r.reasons.map(reason=>`<p>${esc(reason)}</p>`).join('')}</div>`).join('');
      q('.browser-import-explanation',modal).textContent=importActive(j)?'You can close this panel and keep browsing. Annuler stops further copying; data already copied stays in this profile.':j.state==='cancelled'?'Data copied before cancellation remains in this profile.': 'Copied cookies may still require you to sign in on the website. The source browser is unchanged.';
      const stop=q('#import-stop',modal),retry=q('#import-go',modal);stop.hidden=!importActive(j);stop.disabled=j.state==='cancelling';stop.textContent=j.state==='cancelling'?'Annulerling…':'Annuler import';retry.hidden=importActive(j);retry.disabled=importActive(j);retry.textContent=['failed','partial','cancelled'].includes(j.state)?'Review and retry':'Import again';
    };
    importView=update;
  ...
  async function renderImport() {
    ...
    const modal=dialog('modal modal--browser', `<div class="modal-head"><span class="title">Import from your browser</span></div>
      <div class="modal-body browser-profile-body">Chargement…</div>
      <div class="modal-foot"><button class="btn" id="import-cancel">Annuler</button><button class="btn btn--go browser-import-go" id="import-go" disabled>Choose destination</button></div>`);
    const current=()=>state.overlay===o && modal.isConnected;
    q('#import-cancel',modal).onclick=close;
    ...
    const host=q('.browser-profile-body',modal);
    if(!r.profiles?.length){host.textContent='Create a ScalAI profile first.';return;}
    ...
    host.innerHTML=`<label class="field-label">From<select id="import-source"><option value=""${source?'':' selected'} disabled>${sources.length?'Choose a browser profile':'No browser profile found'}</option>${sources.map(s=>`<option value="${esc(s.id)}"${s.id===source?.id?' selected':''}>${esc(s.browser)} · ${esc(s.name)}</option>`).join('')}</select></label>
      ...
      <label class="field-label">Into<select id="import-destination"><option value=""${dest?'':' selected'} disabled>Choose a ScalAI profile</option>${r.profiles.map(p=>`<option value="${esc(p.id)}"${p.id===dest?.id?' selected':''}>ScalAI · ${esc(p.name)}</option>`).join('')}</select></label>
      <p class="note">Allow Keychain access if macOS asks.</p>
      <label class="browser-check"><input type="checkbox" id="import-passwords"${checked('passwords')}><span>Enregistrerd passwords</span></label>
  ...
  function settingsHtml() { return browserParamètresHtml(); }
  function wireParamètres(modal) {
    const o=state.overlay;
    if (!Object.hasOwn(o,'browserPanelId')) o.browserPanelId=state.panels.find(p=>p.id===state.activeId && p.kind==='browser')?.id;
    return wireBrowserParamètres(modal, { api, panelId:o.browserPanelId, profileId:o.browserProfileId,
  ...
  return { open, mount, restore, decorate, schedule, clearNotes, newBrowser, renderNew, renderNote, settingsHtml, wireParamètres, renderProfiles, renderImport, inbox, hasPending:p=>annotations.hasPending(p), canFermer:p=>annotations.canFermer(p), removeNotes:id=>annotations.removeTab(id) };
}
