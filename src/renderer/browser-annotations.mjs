// Pending annotation content exists only in the trusted ScalAI renderer.
...
export function createBrowserAnnotations({ api, esc, icon, selection, insertAnnotation, sessions = () => [], toast, dictation, onChange = () => {}, confirmDiscard, focus = () => {} }) {
  ...
  function edit(p, value) {
    ...
    q('[data-comment="mic"]', bubble).onclick = () => {
      ...
      if (!dictation?.start) { status.textContent = 'Dictation is unavailable. Configure Voice in Paramètres.'; touch(); return; }
  ...
  return { mount, start, stop, toggle, isActive: (id) => !!mounts.get(typeof id === 'object' ? id.id : id)?.active, edit, handleEvent, review, store, cancel,
    ...
    async canFermer(p) { const notes = mounts.has(p.id) ? store.tab(p.id) : store.list(p); const hasDraft = editing && (editing.panelId === p.id || mounts.get(editing.panelId)?.panel.owner === p.id); return approveDiscard(notes.filter(pending).length + (hasDraft ? draftPending(editing.panelId) : 0)); },
