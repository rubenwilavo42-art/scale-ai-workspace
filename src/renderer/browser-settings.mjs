// Paramètres content only: the app owns its modal, navigation and profile flows.
const esc = (value) => String(value ?? '').replace(/[&<>"']/g, (c) => ({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;' }[c]));
export function browserParamètresHtml() {
...
export function browserParamètresContent(status, actions = {}) {
  ...
  const selection = `<section class="bs-section" aria-labelledby="browser-selection-heading">
    <label class="field-label" id="browser-selection-heading" for="browser-settings-profile">Paramètres for profile</label>
    ...
    <p class="bs-note">Nouvel onglet du navigateurs use ${esc(defaultProfile?.name || 'the selected profile')}. Existing tabs keep their profiles.</p>
  ...
  return selection + `<section class="bs-section" aria-labelledby="browser-blank-heading">
    ...
    <p class="bs-note">System follows ScalAI’s theme, and websites follow it too.</p>
  </section>
  <section class="bs-section" aria-labelledby="browser-download-heading">
    ...
    <label class="bs-toggle"><input type="radio" name="browser-download" id="browser-download-auto" value="auto"${download === 'auto' ? ' checked' : ''}><span>Enregistrer to Downloads</span></label>
  </section>
...
export async function wireBrowserParamètres(modal, options) {
  const { api, onError = () => {} } = options;
  const host = modal.querySelector('#browser-settings-body');
  if (!host) return;
  const token = {}; host._browserParamètresRequest = token;
  const current = () => host.isConnected && host._browserParamètresRequest === token;
  try {
    ...
    host.innerHTML = browserParamètresContent(status, { ...options, profileId });
    const refresh = () => current() ? wireBrowserParamètres(modal, { ...options, profileId }) : undefined;
    const picker = host.querySelector('#browser-settings-profile');
    if (picker) picker.onchange = () => {
      options.onProfileChange?.(picker.value);
      return wireBrowserParamètres(modal, { ...options, profileId: picker.value });
    };
    let saving = false;
    ...
    host.querySelector('button').onclick = () => wireBrowserParamètres(modal, options);
  }
}
