...
const KIND_LABEL = {
  ...
  search: 'Search', delete: 'Supprimer', move: 'Move', think: 'Think', other: 'Tool',
};
...
export function createTranscript(container, opts) {
  ...
  return {
    ...
    permission(params, answer, onRetire) {
      ...
      const title = (params.toolCall && params.toolCall.title) || 'Action de l’agent';
      const opts = params.options || [];
      const allow = opts.find((x) => x.kind === 'allow_once') || opts.find((x) => String(x.kind || '').startsWith('allow'));
      const deny = opts.find((x) => x.kind === 'reject_once') || opts.find((x) => String(x.kind || '').startsWith('reject'));
      const el = block(`<div class="cw-perm"><div class="q">Autorisation requise</div>${owned ? '' : `<code>${esc(title)}</code>`}
        <div class="col"><button class="cw-btn row" data-decision="deny">Refuser</button><button class="cw-btn row" data-decision="once">Autoriser une fois</button><button class="cw-btn ap" data-decision="session">Autoriser pour la session</button></div></div>`);
      ...
      el.querySelectorAll('[data-decision]').forEach((b) => {
        b.onclick = (e) => {
          ...
          const op = decision === 'deny' ? deny : allow;
          if (!op) { state2.answered = false; return; }
          el.querySelector('.cw-perm').outerHTML =
            `<div class="cw-settle ${decision !== 'deny' ? 'ok' : ''}"><b>${decision !== 'deny' ? '✓' : '✗'}</b> ${esc(title)} — ${decision === 'session' ? 'Autorisé pour la session' : decision === 'once' ? 'Autorisé une fois' : 'Refusé'}</div>`;
          answer(op.optionId);
        };
      });
      setTimeout(() => {
