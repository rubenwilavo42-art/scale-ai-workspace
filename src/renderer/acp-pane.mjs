...
export function mountChatPane(p, rec, hooks) {
  ...
  empty.querySelector('.cw-empty-name').textContent = p.title || 'Nouvelle session';
  ...
  const composer = createComposer(compHost, {
    ...
    getBuiltins: () => {
      ...
      rows.push({ name: 'config', description: 'Paramètres de la session' });
  ...
  composer.setCompétences((window.__namiCompétences || []).length ? window.__namiCompétences : [
  ...
  const offExit = api.onAcpExit(({ id, code }) => {
    ...
    transcript.error('L’agent s’est arrêté (code de sortie ' + code + '). Vous pouvez relancer cette session.');
  });
  const transport = {
  ...
  const sessionPermissionGrants = new Set();
  const permissionFingerprint = (params) => {
    const tc = params?.toolCall || {};
    return JSON.stringify([tc.title || '', tc.kind || '', (params?.options || []).map(o => [o.kind, o.optionId]).join('|')]);
  };
  const pickPermissionOption = (params, kind) => {
    const opts = params?.options || [];
    if (kind === 'once') return opts.find(o => o.kind === 'allow_once') || opts.find(o => String(o.kind || '').startsWith('allow'));
    if (kind === 'deny') return opts.find(o => o.kind === 'reject_once') || opts.find(o => String(o.kind || '').startsWith('reject'));
    return opts.find(o => o.kind === 'allow_once') || opts.find(o => String(o.kind || '').startsWith('allow'));
  };
  ...
  const client = createAcpClient(transport, {
    ...
    onPermission: (params, reply) => {
      ...
      const key = permissionFingerprint(params);
      if (sessionPermissionGrants.has(key)) {
        const op = pickPermissionOption(params, 'session');
        if (op) { reply(op.optionId); return; }
      }
      // Normalize ACP's provider-specific option list to ScalAI's three choices.
      // 'session' is intentionally local: we remember only this session and
      // continue replying with the protocol's allow_once option.
      const answer = (decision) => {
        const op = pickPermissionOption(params, decision);
        if (!op) { reply(pickPermissionOption(params, 'deny')?.optionId); return; }
        if (decision === 'session') sessionPermissionGrants.add(key);
        reply(op.optionId);
        if (hooks.settled) hooks.settled(p);
      };
      transcript.permission(params, (optionId) => {
        // Legacy renderer cards can still return the raw ACP option id. Treat
        // allow_once as one-time and every rejecting option as deny.
        const op = (params.options || []).find(o => o.optionId === optionId);
        if (op?.kind === 'allow_once') answer('once');
        else if (op?.kind && String(op.kind).startsWith('allow')) answer('session');
        else answer('deny');
      }, () => answer('deny'));
    },
