...
// ScalAI — the agent workbench, par ScalAI (renderer, terminal-first).
...
import { isFile as isFilePanel, isSession as isSessionPanel, ownerFor, groupRail, previewToReplace, keep as keepFile, orphan as orphanFichiers, moveTo as moveFile, splitAfter, focusSplit, splitLayout, ownerIndexes, resolveOwners } from './desk-view.mjs';
...
import { sessionAction, sessionState, restartOptions } from './session-lifecycle.mjs';
import { MAX_COMPARE_SESSIONS, comparableSessions, normalizeCompareSelection, compareLayout, canCompare } from './multi-agent-workspace.mjs';
import { buildDeskMeta, restoreDeskMeta } from './desk-persistence.mjs';
import { addContext, contextIndexes, restoreContextIndexes, normalizeContextEntries } from './session-context.mjs';

const api = window.scalai;

// Agent execution permissions. Terminal launches are gated by main-process policy;
// this sheet is deliberately renderer-only UI, while the decision is enforced in main.
function openAgentPermissionRequest(req) {
  const existing = document.querySelector('.scalai-permission-sheet');
  if (existing) existing.remove();
  const modal = overlay('modal modal--permission scalai-permission-sheet', `
    <div class="modal-head"><span class="title">Autorisation requise</span><span class="note">ScalAI protège votre espace de travail</span></div>
    <div class="modal-body permission-sheet">
      <p><strong>${esc(req.agentId || 'Agent')}</strong> souhaite lancer une action dans :</p>
      <div class="permission-path">${esc(req.cwd || req.workspaceRoot || 'workspace')}</div>
      <p class="permission-label">Commande</p>
      <pre class="permission-command">${esc(req.command || '(commande non fournie)')}</pre>
      <p class="permission-note">Les identifiants enregistrés ne sont jamais transmis à l’interface. Seul l’agent autorisé peut recevoir les credentials prévus pour lui.</p>
    </div>
    <div class="modal-foot permission-actions">
      <button class="btn" data-permission="deny">Refuser</button>
      <button class="btn" data-permission="once">Autoriser une fois</button>
      <button class="btn btn--go" data-permission="session">Autoriser pour la session</button>
    </div>`);
  modal.querySelectorAll('[data-permission]').forEach((b) => b.onclick = async () => {
    const decision = b.dataset.permission;
    b.disabled = true;
    await api.resolvePermission(req.id, decision);
    closeOverlay();
  });
}
api.onPermissionRequest?.(openAgentPermissionRequest);
api.onPermissionDenied?.((ev) => toast(ev?.error || 'Action refusée.'));
api.onPermissionError?.((ev) => toast(ev?.error || 'Action bloquée par le contrôle du workspace.'));
...
const THEME_KEY = 'daiscalai-theme';
...
function normalizeThème(name) { return THEME_NAMES.includes(name) ? name : DEFAULT_THEME; }
function currentThème() {
  const t = document.body.dataset.theme;
  return t === undefined ? 'paper' : normalizeThème(t);
}
function xtermThème() { return XTERM_THEMES[currentThème()]; }
function statusColors() { return STATUS_COLORS[currentThème()]; }
...
// It is what makes ScalAI's chrome look hand-made and it is the worst thing about
...
function applyThèmeAttrs(name) {
  name = normalizeThème(name);
...
let themeEnregistrerVersion = 0;
function setThème(name, persistIt = true) {
  name = normalizeThème(name);
  applyThèmeAttrs(name);
  if (api.themeApplied) api.themeApplied(name);
  const version = ++themeEnregistrerVersion;
  ...
  if (persistIt && !S.review && api.themeSet) {
    saved = Promise.resolve(api.themeSet(name)).then((result) => {
      ...
      if (version === themeEnregistrerVersion) { try { localStorage.setItem(THEME_KEY, name); } catch (_) {} }
    }).catch(() => toast('L’apparence a changé, mais n’a pas pu être enregistrée pour le prochain lancement.'));
  }
  tileEls.forEach((t, id) => {
    if (!t.term) return;
    t.term.options.theme = xtermThème();
  ...
  if (els.grid) { renderAll(); requestAnimationFrame(positionThèmePop); }
  return saved;
}
...
try { applyThèmeAttrs(localStorage.getItem(THEME_KEY) || DEFAULT_THEME); } catch (_) { applyThèmeAttrs(DEFAULT_THEME); }
...
const S = {
  ...
  version: '', updatedAt: null,        // shown in Paramètres → About, filled at boot
...
const browsers = createBrowserPane({ api, state: S, tiles: tileEls, uid, esc, helpIcon, isFile: isFilePanel, isSession: isSessionPanel,
  ...
  selection: openSelectionDraft, insertAnnotation, sessions: () => S.panels.filter(isSessionPanel).filter(p=>!p.exited), panelIcon:panelChip, settings: openParamètres, closePanel, dictation: { start: startAnnotationDictation },
...
async function insertAnnotation(payload,destinations) {
  ...
  for(const id of destinations) {
    ...
    if(!p||p.exited||!rec){failed.push({id,error:'La session est fermée.'});continue;}
    ...
    if(rec.insertSessionDraft) {
      ...
      if(payload.image)text+=`\n\nScreenshot file reference: ${JSON.stringify(payload.image.path)}\nScalAI image ID: ${payload.image.id} (available through nami_read_annotation_image when connected).`;
...
// ---- OS file drops ---------------------------------------------------------
function isFileDrag(e) { return dragTypes(e).includes('Fichiers'); }
...
function dropFichiersOnPanel(p, paths) {
  if (p.kind === 'editor' || p.kind === 'viewer') { paths.forEach((f) => openFile(f, { pin: true })); return; }
  if (p.kind === 'acp') {
    const t = tileEls.get(p.id);
    p.contextFiles = paths.reduce((list, f) => addContext(list, { path: f, kind: 'file', name: baseNameOf(f) }), p.contextFiles || []);
...
function dropPathOnPanel(p, path, isDir) {
  ...
  p.contextFiles = addContext(p.contextFiles, { path, kind: isDir ? 'directory' : 'file', name: baseNameOf(path) });
...
(async function boot() {
  ...
  // the lockup is never laid out twice — the stack is sized by ScalAI above it,
  ...
  setThème(b.themeArg || b.theme || DEFAULT_THEME, false);
  if (!S.review) { try { localStorage.setItem(THEME_KEY, currentThème()); } catch (_) {} }
  ...
  // A one-shot command ScalAI ran on the user's behalf has landed. The shell is
  ...
  api.onTermExit(({ id, code, note }) => {
    ...
    // main writes the note, because only it knows whether ScalAI ended this
...
function showScene(name) {
  ...
  if (what === 'settings') return openParamètres(step || 'voice');
  ...
  // The folder-first card asks where a session should run when aucun dossier is
  ...
  if (what === 'update') {
    ...
    offerUpdate({ version: staged ? '0.2.0' : (step || '0.2.0'), url: 'https://example.test/ScalAI.dmg' });
  ...
  if (what === 'theme') return toggleThèmePop();
  if (what === 'term-scroll') {
...
// ===========================================================================
// Every ScalAI item in the application menu arrives here as a string. The rule
...
// close *pane* instead of Fermer Window, which is what the conventional menu
...
function runMenuCommand(cmd) {
  ...
  if (what === 'about') return openParamètres('about');
  if (what === 'settings') return openParamètres(arg || 'voice');
  if (what === 'update-check') {
    openParamètres('about');
  ...
  if (what === 'new-file' || what === 'new-folder') {
    if (!S.project) { toast('Ouvrez d’abord un dossier.'); return openFolderDialog(); }
    S.railTab = 'workspace'; renderRail();
    return openFsName(what === 'new-file' ? 'file' : 'folder', S.project.path);
  }
  if (what === 'save') { if (!saveActive()) toast('Rien à enregistrer ici.'); return undefined; }
  if (what === 'reveal') {
    const p = activeFilePanel();
    if (!p) { toast('Ouvrez d’abord un fichier.'); return undefined; }
    return api.revealFile(p.filePath);
  }
  if (what === 'close-pane') return closeActive();
  if (what === 'dictate') {
    const p = S.panels.find((x) => x.id === S.activeId);
    if (!p) { toast('Démarrez d’abord une session.'); return undefined; }
    return toggleMic(p);
  }
  if (what === 'rail') {
  ...
  if (what === 'theme') return setThème(arg);
...
function closeActive() {
  if (S.overlay && S.overlay.type === 'peek') requestFermerPeek();
...
function buildShell() {
    ...
    <div class="desk"><div class="sheet">
      ...
      <div class="topbar">
        <div class="brand">
          <span class="brand-mark">
            ...
            <span class="brand-stack">
              <span class="brand-name">ScalAI</span>
        ...
        <div class="topbar-right">
          ...
          <div class="theme-zone" id="theme-zone"><button class="btn" id="btn-theme" title="Thème"><span class="uni-i">◐</span><span class="pix-i">${pixIcon('theme')}</span></button></div>
          <button class="btn btn-set" id="btn-settings" title="Paramètres ⌘,"><span class="uni-i">⚙</span><span class="pix-i">${pixIcon('settings')}</span></button>
          <div class="viewsw" id="viewsw" role="group" aria-label="Vue de l’espace de travail" title="Desk: every card on a grid. Split: one session beside one of its files.">
  ...
  q('#btn-shortcuts').onclick = () => openParamètres('shortcuts');
  q('#btn-theme').onclick = (e) => { e.stopPropagation(); toggleThèmePop(); };
  q('#btn-settings').onclick = () => openParamètres();
  ...
  syncDeskColumns();
  window.addEventListener('resize', () => { syncDeskColumns(); positionThèmePop(); });
...
function onGlobalKey(e) {
  ...
  // keyboard. ⌘⌫ and not a bare ⌫ is the whole point: Supprimer on its own is one
  ...
  if (meta && (e.key === 'Backspace' || e.key === 'Supprimer') && inTree && (!S.overlay || peeking)) {
  ...
  if (meta && e.key === ',') { e.preventDefault(); openParamètres(); return; }
  ...
  if (e.key === 'Escape') { if (S.overlay && S.overlay.type === 'peek') { requestFermerPeek(); } else if (S.overlay) { closeOverlay(); } else if (S.expandedId) { S.expandedId = null; renderGrid(); } }
}
...
const THEME_OPTIONS = [
  { id: 'glass', name: 'Clair', desc: 'l’identité ScalAI classique' },
  { id: 'dusk', name: 'Sombre', desc: 'la même identité sur fond sombre' },
];
function positionThèmePop() {
...
function toggleThèmePop() {
  ...
  pop.innerHTML = `<div class="pop-label">Appearance</div>` + THEME_OPTIONS.map((t) =>
    `<button class="theme-opt${currentThème() === t.id ? ' picked' : ''}" data-theme-id="${t.id}" aria-pressed="${currentThème() === t.id}">
  ...
  pop.querySelectorAll('.theme-opt').forEach((b) => {
    b.onclick = () => {
      setThème(b.dataset.themeId);
      pop.querySelectorAll('.theme-opt').forEach((o) => {
        const picked = o.dataset.themeId === currentThème();
...
function refreshWorkspaceRail(c) {
  ...
  if (!p) { wrap.innerHTML = '<div class="rail-empty">Ouvrez un dossier (⌘O) pour parcourir et modifier les fichiers.</div>'; c.appendChild(wrap); return; }
  ...
  plus.className = 'tree-new'; plus.title = 'Nouveau fichier ou dossier';
...
// The menu opened once per session and Duplicate, Copier le chemin and Move to Trash
...
async function addPathToSession(path, isDir) {
  ...
  if (!live.length) { toast('Ouvrez d’abord une session.'); return; }
...
function tileTarget(p) {
  ...
  if (!live.length) { toast('Ouvrez d’abord une session.'); return null; }
...
// Leave ScalAI for the Mac's browser: a saved HTML file through the file
...
async function openOutside(p) {
  ...
  else toast('Rien à ouvrir pour le moment.');
}
function tileMenu(p) {
...
function treeMenu(n, parentDir) {
  ...
  if (n.kind === 'file' && fileKind(n.path) === 'html') {
    items.push({ label: 'Ouvrir dans le navigateur', run: () => openFileInBrowser(n.path) });
    items.push({ label: 'Ouvrir dans Chrome ↗', run: () => openOutside({ filePath: n.path }) });
  }
  ...
  items.push({ label: 'Ouvrir dans une nouvelle fenêtre', run: () => {
  ...
  items.push({ label: 'Copier le chemin', run: async () => {
  ...
  // and the Trash is recoverable. The library card's Supprimer keeps its armed
  // second click because it sits next to Enregistrer.
...
function availabilityTag(i) {
  ...
  if (i.availability === 'agent') {
    ...
    return { text: who + ' only', tone: 'mute', title: `${who} reads this folder itself. ScalAI's sessions here won't see it unless you copy it in.` };
  }
...
// A `## Compétences` heading the user wrote themselves. ScalAI appends below it rather
...
// making. But two Compétences sections in one file is worth mentioning once.
...
function appendForeignNote(list) {
  ...
  note.innerHTML = `<span class="pn-msg">AGENTS.md also has a Compétences section you wrote. ScalAI left it alone and put its own list below — tidy up whenever you like.</span>
    <button class="pn-x" title="Compris">✕</button>`;
...
function appendPointerBar(list) {
  ...
  if ((st.missingFichiers || []).length) bits.push(`${st.missingFichiers.join(' + ')} missing`);
...
async function writePointers(btn) {
  const dir = S.project && S.project.path;
  if (!dir) { toast('Ouvrez d’abord un dossier.'); return; }
...
function refreshLibraryRail(c) {
  ...
  const head = document.createElement('div'); head.className = 'rail-head';
  head.innerHTML = `<span class="title">Bibliothèque</span>
  ...
  search.className = 'lib-search'; search.placeholder = 'Filtrer la bibliothèque…'; search.value = S.library.q;
  ...
  if (!S.library.loaded) { const e = document.createElement('div'); e.className = 'rail-empty'; e.textContent = 'Analyse en cours…'; list.appendChild(e); return; }
...
// With aucun dossier there is exactly one thing worth saying, and it is the thing
...
function renderFooter() { els.footerPath.textContent = S.project ? S.project.pathShort : 'aucun dossier open'; }
...
function statusMeta(p) {
  ...
  if (p.kind === 'acp') {
    ...
    if (p.stopping) return { label: 'stopping', color: c.warn };
...
function emptyDeskHtml() {
  ...
  const open = first
    ? `<button class="btn lane-cta" id="lane-open">Choisir une session existante<span class="kb"> ⌘O</span></button>`
    : `<button class="btn btn--go lane-cta" id="lane-open">＋ Ouvrir un dossier<span class="kb"> ⌘O</span></button>`;
  return `<div class="lane-empty"><div class="polaroid">aucun dossier</div>
      ...
      <button class="lane-tour" id="lane-tour">New to ScalAI? Start here</button></div></div>`;
}

// Hands off to the save panel in main, then through the ordinary switch path —
// a folder ScalAI made is not a special kind of folder once it exists.
...
function openAgentCompare() {
  const sessions = comparableSessions(S.panels);
  if (sessions.length < 2) { toast('Il faut au moins deux sessions actives pour comparer.'); return; }
  const selected = normalizeCompareSelection(S.panels, S.compareIds.length ? S.compareIds : sessions.slice(0, 2).map((p) => p.id));
  S.overlay = { type: 'agent-compare', ids: selected };
  renderOverlay();
}
function renderAgentCompare() {
  const o = S.overlay;
  const sessions = comparableSessions(S.panels);
  const selected = normalizeCompareSelection(S.panels, o.ids);
  o.ids = selected;
  const modal = overlay('modal modal--agent-compare', `<div class="modal-head"><span class="title">Comparer les agents</span><span class="note">2 à ${MAX_COMPARE_SESSIONS} sessions</span></div>
    <div class="modal-body selection-sheet"><p class="note">Choisissez les sessions à afficher côte à côte. Chaque agent conserve son propre terminal, contexte et état.</p>
    <div class="compare-recipients">${sessions.map((s) => `<label class="compare-recipient"><input type="checkbox" data-compare-session="${esc(s.id)}"${selected.includes(s.id) ? ' checked' : ''}${selected.length >= MAX_COMPARE_SESSIONS && !selected.includes(s.id) ? ' disabled' : ''}><span class="compare-recipient-icon">${iconSvg(iconKeyFor(s.agentId || s.title)) || esc(code2(s.title))}</span><span><strong>${esc(s.title)}</strong><small>${esc(s.cwd || S.project?.path || '')}</small></span><span class="compare-recipient-state">${esc(sessionState(s))}</span></label>`).join('')}</div></div>
    <div class="modal-foot"><span class="note" id="compare-count">${selected.length} sélectionnée${selected.length > 1 ? 's' : ''}</span><button class="btn" id="compare-cancel">Annuler</button><button class="btn btn--go" id="compare-open" ${selected.length < 2 ? 'disabled' : ''}>Afficher côte à côte</button></div>`);
  const update = () => {
    o.ids = normalizeCompareSelection(S.panels, [...modal.querySelectorAll('[data-compare-session]:checked')].map((x) => x.dataset.compareSession));
    const n = o.ids.length;
    q('#compare-count', modal).textContent = `${n} sélectionnée${n > 1 ? 's' : ''}`;
    const btn = q('#compare-open', modal); btn.disabled = n < 2;
    modal.querySelectorAll('[data-compare-session]').forEach((x) => { x.disabled = n >= MAX_COMPARE_SESSIONS && !x.checked; });
  };
  modal.querySelectorAll('[data-compare-session]').forEach((b) => b.onchange = update);
  q('#compare-cancel', modal).onclick = closeOverlay;
  q('#compare-open', modal).onclick = () => {
    if (!canCompare(o.ids, S.panels)) return;
    S.compareIds = normalizeCompareSelection(S.panels, o.ids);
    S.expandedId = null;
    S.view = 'desk';
    closeOverlay();
    renderGrid(); renderRail(); renderHeader(); savePanels();
    const first = S.compareIds[0]; if (first) focusPanel(first, false, { preserveLayout: true });
  };
}
function clearAgentCompare() {
  S.compareIds = [];
  if (els.grid) { els.grid.classList.remove('compare-mode'); els.grid.style.removeProperty('--compare-cols'); }
}

function renderWorkspaceStrip() {
  const main = els.grid && els.grid.parentElement;
  if (!main) return;
  let strip = q('.workspace-strip', main);
  if (!S.panels.some(isSessionPanel)) {
  ...
  if (!strip) {
    ...
    main.insertBefore(strip, els.grid);
  }
  const sessions = S.panels.filter(isSessionPanel);
  const live = sessions.filter((p) => !p.exited && !p.commandDone);
  const cwd = S.project?.path ? shortHome(S.project.path) : 'Aucun dossier';
  strip.innerHTML = `<div class="workspace-strip-main">
    <span class="workspace-strip-label">Espace de travail</span>
    <span class="workspace-strip-path" title="${esc(S.project?.path || '')}">${esc(cwd)}</span>
    <span class="workspace-strip-count">${sessions.length} session${sessions.length > 1 ? 's' : ''}</span>
  </div>
  <div class="workspace-strip-agents" aria-label="Sessions agents">
    ${sessions.map((p) => {
      const key = iconKeyFor(p.agentId || p.title) || '';
      const icon = iconSvg(key);
      const active = p.id === S.activeId;
      const state = p.exited || p.commandDone ? 'terminee' : p.attention ? 'attention' : 'active';
      return `<button class="workspace-agent-chip ${active ? 'active' : ''} ${state}" data-session-id="${esc(p.id)}" title="${esc(p.title)}">
        <span class="workspace-agent-icon">${icon || esc(code2(p.title))}</span>
        <span class="workspace-agent-name">${esc(p.title)}</span>
        <span class="workspace-agent-state" aria-hidden="true"></span>
      </button>`;
    }).join('')}
  </div>
  <div class="workspace-strip-actions">
    ${sessions.length >= 2 ? '<button class="workspace-strip-compare" id="workspace-compare-agents" title="Comparer plusieurs agents">Comparer</button>' : ''}
    <button class="workspace-strip-add" id="workspace-add-agent" title="Ajouter un agent">＋</button>
  </div>`;
  strip.querySelectorAll('[data-session-id]').forEach((b) => {
    b.onclick = () => focusPanel(b.dataset.sessionId, true);
  });
  q('#workspace-add-agent', strip)?.addEventListener('click', () => openLauncher());
  q('#workspace-compare-agents', strip)?.addEventListener('click', () => openAgentCompare());
}
...
function renderGrid() {
  if (!S.panels.length) {
    ...
    // alone, from when that was the only session ScalAI could start.
    els.grid.innerHTML = S.project
      ...
      <button class="btn btn--go lane-cta" id="lane-new">＋ Nouvelle session<span class="kb"> ⌘N</span></button></div></div>`
      : emptyDeskHtml();
  ...
  const liveCompare = normalizeCompareSelection(S.panels, S.compareIds);
  S.compareIds = liveCompare;
  const compareMode = canCompare(liveCompare, S.panels);
  els.grid.classList.toggle('compare-mode', compareMode);
  els.grid.style.setProperty('--compare-cols', String(compareLayout(liveCompare.length).columns));
  ...
  for (const p of S.panels) {
    ...
    t.root.classList.toggle('compare-tile', compareMode && liveCompare.includes(p.id));
    t.root.classList.toggle('compare-hidden', compareMode && !liveCompare.includes(p.id));
    refreshTileHead(p);
...
function renderSplit() {
  ...
  place(q('.pane-files', pv), S.split.fileId, sess ? `No files with ${sess.title} yet — open one from the Workspace tab` : 'Ouvrez un fichier depuis l’onglet Espace de travail');
}
...
// Enregistrerd global and per-tile sizes win over the theme's fresh-session default.
const TERM_FONT_KEY = 'dainami-term-fontsize';
function defaultTermFont() {
  const fallback = currentThème() === 'operator' ? 14 : TERM_FONT_DEFAULT;
...
function spawnTerminalTwin(p, draft) {
  ...
  if (p.agentId === 'claude' || (a && a.kind === 'claude')) {
    ...
    toast('Ouvrez-le avec ⌘N — nouvelle session, puis choisissez l’agent.');
    return;
  }
...
async function stopSession(p) {
  if (!isSessionPanel(p) || p.exited || p.stopping) return;
  p.stopping = true;
  refreshTileHead(p); renderRail(); renderHeader(); savePanels();
  try { await api.termKill({ id: p.id }); }
  catch (_) { p.stopping = false; refreshTileHead(p); renderRail(); toast('Impossible d’arrêter cette session.'); }
}

function restartSession(p) {
  const opts = restartOptions(p);
  if (!opts) return;
  // Keep the old transcript in place until the new process has been created.
  // The close path disposes the old PTY and removes only that panel.
  closePanel(p.id, { silent: true });
  const next = startPanel(opts);
  if (next) toast(`${p.title} — nouvelle session`);
}

function duplicateSession(p) {
  if (!isSessionPanel(p) || p.exited) return;
  const a = (S.agents || []).find((x) => x.id === p.agentId);
  let opts = restartOptions(p);
  if (!opts) return;
  // A duplicate is a genuinely new conversation: never reuse resume ids or a
  // seed that would make a CLI re-enter the old conversation.
  delete opts.sid; delete opts.acpSid; delete opts.cont; delete opts.seed;
  opts.title = `${p.title} — copie`;
  opts.titleSource = 'prompt';
  if (a && a.found) opts = { ...terminalAgentOptions(a), ...opts, title: opts.title, code: opts.code };
  const next = startPanel(opts);
  if (next) toast(`${p.title} — copie créée`);
}

function sessionControl(p) {
  const action = sessionAction(p);
  if (action === 'stop') return { label: 'Arrêter la session', icon: '■', run: () => stopSession(p) };
  if (action === 'restart') return { label: 'Relancer la session', icon: '↻', run: () => restartSession(p) };
  return null;
}
...
function mountTile(p) {
  ...
  root.innerHTML = `<div class="tile-head" draggable="true">
      ...
      ${isSessionPanel(p) ? `<button class="t-btn t-session-control" title="${esc(sessionControl(p)?.label || 'Session')}" aria-label="${esc(sessionControl(p)?.label || 'Session')}"${sessionAction(p) === 'none' ? ' disabled' : ''}>${sessionControl(p)?.icon || '•'}</button>` : ''}
      ...
      <button class="t-btn t-close" title="Fermer"><span class="uni-i">✕</span><span class="pix-i">${pixIcon('close')}</span></button>
  ...
  const sessionCtl = q('.t-session-control', head);
  if (sessionCtl) sessionCtl.onclick = (e) => { e.stopPropagation(); sessionControl(p)?.run(); };
  ...
  if (isSessionPanel(p)) head.oncontextmenu = (e) => {
    e.preventDefault();
    const ctl = sessionControl(p);
    const items = [
      { label: 'Ajouter un navigateur…', run: () => browsers.newBrowser(p.id) },
      { label: 'Dupliquer la session', off: !!p.exited, run: () => duplicateSession(p) },
    ];
    if (ctl) items.push({ label: ctl.label, run: ctl.run });
  ...
  root.addEventListener('drop', (e) => {
    ...
    if (paths.length) return dropFichiersOnPanel(p, paths);
...
function refreshTileHead(p) {
  ...
  const contextCount = normalizeContextEntries(p.contextFiles).length;
  q('.t-sub', t.head).textContent = kindLabel(p) + (owner ? ' · ' + shorten(owner.title, 22) : '') + (contextCount ? ` · ${contextCount} contexte${contextCount > 1 ? 's' : ''}` : '');
  q('.t-status .lbl', t.head).textContent = m.label;
  t.statusDot.style.background = m.color;
  const ctl = q('.t-session-control', t.head);
  if (ctl) { const action = sessionControl(p); ctl.textContent = action?.icon || '•'; ctl.title = action?.label || 'Session'; ctl.setAttribute('aria-label', action?.label || 'Session'); ctl.disabled = !action; ctl.classList.toggle('is-restart', action?.icon === '↻'); }
...
function mountTerminal(p, rec) {
  const term = new Terminal({
    ...
    lineHeight: 1.45,
    theme: xtermThème(), cursorBlink: true, allowTransparency: true, allowProposedApi: true,
    scrollback: 6000,
...
function openSelectionDraft(p, selection, destination) {
  ...
  const sessions = S.panels.filter(isSessionPanel).filter((s) => !s.exited);
  if (!sessions.length) { toast('Ouvrez d’abord une session.'); return; }
...
function renderSelectionDraft() {
  ...
  const modal = overlay('modal modal--selection', `<div class="modal-head"><span class="title">Insert selection into session</span></div>
    ...
    <div class="modal-foot"><span class="note">Inserts into the session input without submitting.</span><button class="btn" id="selection-cancel">Annuler</button><button class="btn btn--go" id="selection-add">Insert into session</button></div>`);
...
async function collectBases(term, p, anchorTop) {
  ...
  while (y >= 1 && walked < BASES_ROWS && bases.length < 6) {
    ...
    // No slash, aucun dossier — skip the scan without paying for it.
    if (run.text.indexOf('/') !== -1) {
      for (const b of basesFromText(run.text, 6)) {
        ...
        // aucun dossier, and garbage must not burn slots in the pool.
...
function wireTerminalMenu(p, rec) {
  rec.body.addEventListener('contextmenu', (e) => {
    ...
    if (picked?.trim()) { e.preventDefault(); showMenu(e.clientX, e.clientY, [{ label: 'Add selection to session…', run: () => openSelectionDraft({ owner: p.id }, { reference: p.title + ' (terminal excerpt)', text: picked }) }, { label: 'Copier la sélection', run: () => copyLinkText(picked) }]); return; }
    ...
    if (hit.link.kind === 'url') items.unshift({ label: 'Open in ScalAI browser', run: () => { browsers.open(urlTarget(hit.link.text), null, p.id); setView('split'); } });
    showMenu(e.clientX, e.clientY, items);
  });
}
...
async function startProcess(p, cols, rows) {
  ...
  await api.termCreate({ id: p.id, cwd: p.cwd, workspaceRoot: S.project?.path || p.cwd, cols, rows, kind: p.kind, command: p.command, program: p.program, args: p.args, seed: p.seed, cont: p.cont, sid: p.sid, acpSid: p.acpSid, name, watchDone: !!p.watchDone, oneShot: !!p.oneShot, purpose: p.purpose, agentId: p.agentId });
}
...
function browserButtonLabel(button, p) {
  if (!button) return;
  button.innerHTML = p && p.dirty ? 'Enregistrer &amp; open in Chrome ↗' : 'Ouvrir dans Chrome ↗';
  button.title = p && p.dirty
    ? 'Enregistrer this page, then open it in Chrome'
...
function mountEditor(p, rec) {
  ...
  wrap.innerHTML = `<div class="disk-bar" hidden>
    ...
    <div class="ed-bar"><span class="ed-path">${esc(shortHome(p.filePath))}</span>${html && !rec.peek ? '<button class="btn ed-browser"></button>' : ''}<button class="btn ed-finder">Finder</button><button class="btn btn--go ed-save">Enregistrer ⌘S</button></div>`;
  ...
  // Fermerd by default: a document opens as a document, not as a form. The
  ...
  const applyMode = () => {
    ...
    if (p.edMode === 'read') {
      if (html) {
        ...
        // opaque origin and cannot reach ScalAI. The injected <base> makes the
        ...
        if (p.dirty) {
          ...
          // because a srcdoc page shares ScalAI's file:// origin and the flag
          ...
          // Enregistrerd → served from scalai-doc://, its own origin. Relative images
          // load, and allow-same-origin is safe: "same origin" is the page's
          // nami-doc origin, cross-origin to ScalAI, so it still cannot reach the
        ...
        // paths through scalai-doc:// (its containment gate refuses .. escapes),
...
async function saveEditor(p) {
  ...
  if (res && res.ok) {
    ...
    toast('Enregistrerd ' + baseNameOf(p.filePath));
    return true;
  }
  toast('Enregistrer failed: ' + (res && res.error || '?'));
  return false;
}
...
function mountViewer(p, rec) {
  ...
  // Served from scalai-doc://, the page's own origin — relative images load and
  // allow-same-origin is safe because that origin is cross-origin to ScalAI (see
...
function mountCard(p, rec) {
  ...
  wrap.innerHTML = `
    ...
    <div class="ed-bar">
      ...
      ${p.item.broken ? '<button class="btn btn--go card-del">Remove this dead link</button>'
        : ro ? ''
        : '<button class="btn card-del">Supprimer</button><button class="btn card-improve">Improve with my agent</button><button class="btn btn--go card-save">Enregistrer ⌘S</button>'}
    </div>`;
  rec.body.appendChild(wrap);
  ...
  if (useBtn) useBtn.onclick = () => {
    ...
    if (!tool) { toast('Aucun agent installé ne peut exécuter ' + p.item.slug + '.'); return; }
    launchAgent(p.item, tool);
  };
  const adoptBtn = q('.card-adopt', wrap);
  if (adoptBtn) adoptBtn.onclick = async () => {
    if (p.dirty) { toast('Enregistrer the card first — the master is lifted from the file.'); return; }
  ...
  if (dupBtn) dupBtn.onclick = async () => {
    ...
    if (!res.ok) { toast(res.error || 'Échec de la copie'); return; }
  ...
  if (impBtn) impBtn.onclick = () => {
    if (p.dirty) { toast('Enregistrer the card first so your agent sees your latest.'); return; }
  ...
  if (delBtn) delBtn.onclick = async () => {
    ...
    const res = await api.librarySupprimer({ filePath: p.item.filePath, projectPath: S.project && S.project.path });
  ...
  // clicking or tabbing anywhere else stands the armed Supprimer back down
  if (delBtn) delBtn.onblur = () => {
    if (!delBtn.dataset.armed) return;
    delete delBtn.dataset.armed; delBtn.textContent = 'Supprimer'; delBtn.classList.remove('armed');
  };
  applyMode();
}
async function saveCard(p) {
  ...
  if (res && res.ok) {
    ...
    refreshTileHead(p); refreshRail(); toast('Enregistrerd ' + p.title);
    loadLibrary(true);
  ...
  } else toast('Enregistrer failed: ' + (res && res.error || '?'));
}
...
function startAnnotationDictation({ onState, onText, onError }) {
  ...
  (async () => {
    ...
    if(cancelled)return;
    try {
      if (recording) throw new Error('Arrêtez la dictée de la session avant d’enregistrer un commentaire.');
      if (!S.stt) throw new Error('Choose a speech provider in Paramètres → Voice first.');
...
async function toggleMic(p) {
  ...
  if (!S.stt) { toast('Pick how ScalAI should hear you.'); return openParamètres('voice'); }
  try {
...
function panelSnapshot() {
  ...
  const size = (p) => {
    ...
    if (isSessionPanel(p)) { const entries = normalizeContextEntries(p.contextFiles); if (entries.length) o.contextFiles = entries; }
  ...
  return S.panels.map((p) => {
    ...
    // desk, quit, and ScalAI piped curl into bash again on the next launch, and
    ...
    return { kind: p.kind, title: p.title, titleSource: p.titleSource, code: p.code, chipKind: p.chipKind, cwd: p.cwd, command: p.command, program: p.program, args: p.args, sid: p.sid, acpSid: p.acpSid, oneShot: p.oneShot, purpose: p.purpose, agentId: p.agentId, watchDone: p.watchDone, ...(normalizeContextEntries(p.contextFiles).length ? { contextFiles: normalizeContextEntries(p.contextFiles) } : {}), ...size(p) };
  });
}
function savePanels() {
  if (S.demo) return;
  ...
  saveTimer = setTimeout(() => {
    const folder = S.project ? S.project.path : null;
    api.savePanels({ panels: panelSnapshot(), folder });
    api.saveDeskMeta?.({ folder, meta: buildDeskMeta(S.panels, { activeId: S.activeId, expandedId: S.expandedId, view: S.view }) });
  }, 400);
}
...
function flushPanels(folder) {
  ...
  clearTimeout(saveTimer);
  const meta = buildDeskMeta(S.panels, { activeId: S.activeId, expandedId: S.expandedId, view: S.view });
  return Promise.all([api.savePanels({ panels: panelSnapshot(), folder: folder || null }), api.saveDeskMeta?.({ folder: folder || null, meta })]);
}
async function restorePanels(snaps) {
  ...
  for (let i = 0; i < snaps.length; i++) if (Array.isArray(snaps[i]?.contextFiles)) contextSnapshot[i] = snaps[i].contextFiles;
  restoreContextIndexes(restored.filter(Boolean), contextSnapshot);
  browsers.restore();
...
function startPanel(opts) {
  ...
  // files its transcript under a project slug aucun dossier can ever resume from.
  ...
  if (!cwd && !['editor', 'viewer', 'card'].includes(opts.kind || 'claude')) {
    toast('Ouvrez d’abord un dossier — les sessions s’exécutent à l’intérieur.');
    return null;
  }
...
function closePanel(id, opts = {}) {
  const p = S.panels.find((x) => x.id === id); if (!p) return;
  if ((p.kind==='browser'||isSessionPanel(p)) && !opts.browserConfirmed && browsers.hasPending(p)) { browsers.canFermer(p).then(ok=>{if(ok)closePanel(id,{...opts,browserConfirmed:true});}); return; }
  ...
  orphanFichiers(S.panels, id); // a closed session's files stay, on the desk
...
function renderFolderFirst() {
  ...
  const modal = overlay('picker-box', `<div class="picker-input"><span class="prompt-mark">＋</span>
    ...
    : `<div class="ff-empty">
        ...
        <button class="btn btn--go" id="ff-pick">Choisir un dossier…</button>
...
function renderLauncher() {
  ...
  const modal = overlay('picker-box', `<div class="picker-input"><span class="prompt-mark">＋</span><span style="font-weight:700">Nouvelle session</span>
    <span style="margin-left:auto;font-size:11px;color:var(--muted)">${S.project ? esc(S.project.name) : 'aucun dossier'}</span></div>
  ...
  // The one vient d’être ajouté sorts to the top. Anything else and the user is handed
  ...
  if (!S.agents) {
    ...
    row.innerHTML = `<span class="col"><span class="desc">Recherche des agents sur ce Mac…</span></span>`;
    list.appendChild(row);
  }
  for (const a of ready) {
    ...
    row.innerHTML = `${chipHtml({ key: iconKeyFor(a.id), code: code2(a.name), kind: 'agent' })}
      ...
      <span class="desc"><span class="ok${st.dot === 'warn' ? ' ok--warn' : ''}">●</span> prêt · ${esc(st.text)}</span></span>
      ${fresh ? '<span class="row-new">vient d’être ajouté</span>' : ''}
      ${manageable ? '<span class="chev" title="Gérer cet agent">›</span>' : ''}`;
    const launch = () => {
      closeOverlay();
      withFolder(() => {
    ...
    if (demoAcp) {
      ...
      tail.innerHTML = '<button class="lc-chat">Chat<span class="lc-beta">bêta</span></button>';
    ...
    row.onclick = async (e) => {
      ...
      if (demoAcp && e.target.closest('.lc-chat')) {
        ...
        toast(a.name + ' \u2014 nouvelle session de chat');
        return;
      }
      launch();
    };
  ...
  if (missing.length) {
    ...
    div.textContent = 'ajouter un agent à ce Mac'; list.appendChild(div);
    ...
    for (const a of missing) {
      ...
      card.innerHTML = `${chipHtml({ key: iconKeyFor(a.id), code: code2(a.name), kind: 'agent' })}
        <span class="ac-name">${esc(a.name)}</span><span class="ac-desc">${esc(a.sub)}</span><span class="ac-go">configurer →</span>`;
...
function renderAgentInstalled(a) {
  ...
  const modal = overlay('setup-box', `
    ...
    <div class="scan-box ag-scan">
      ...
      ${S.agentManager[a.id] && S.agentManager[a.id].version ? `<div class="scan-row"><span class="mark">✓</span><span class="label2">Version</span><span class="value">${esc(S.agentManager[a.id].version)}</span></div>` : `<div class="scan-row"><span class="mark">·</span><span class="label2">Version</span><span class="value">vérification au prochain scan</span></div>`}
    </div>
  ...
  const saveKey = async () => {
    ...
    if (!v) { toast('Collez d’abord le secret.'); return; }
    ...
    if (ga && ga.logoutAfterEnregistrer && lc.logout) {
...
function renderAgentRemove() {
  ...
  const modal = overlay('setup-box', `
    ...
    <div class="setup-actions">
      ...
      <button class="btn" id="ar-keep">${plan && plan.mode === 'none' ? 'Fermer' : 'Keep it'}</button>
    </div>`);
...
function renderAgentInstall(a) {
  ...
  q('#su-run', modal).onclick = () => {
    closeOverlay();
    // oneShot + watchDone: this tile exists to run one command ScalAI chose, and
...
async function finishAgentInstall(p, code) {
  ...
  if (!ok) {
    setTileNote(p, `<span class="tn-tx"><b>${esc(name)} is still not on this Mac.</b>
      ${code === 0 ? 'The command ran to the end but left nothing ScalAI can find — the output above should say why.'
  ...
  setTileNote(p, `<span class="tn-tx"><b>${esc(a.name)} is on this Mac.</b>
    ...
    <span class="tn-bt"><button class="btn btn--go btn--small" id="tn-go">Back to Nouvelle session</button></span>`, 'ok');
  const t = tileEls.get(p.id); if (!t) return;
...
async function ensureDelivered(item, toolId) {
  ...
  // mode, say) reaches copies delivered before it. Marked files are ScalAI's to
...
function deliveryNote(item, toolId, was) {
  ...
  if (was.state === 'theirs') {
    return `${item.slug} on ${tool} — your own ${baseNameOf(was.file)} is there and ScalAI left it alone, `
...
// Nouvelle session. Seed, surface and title are master's `useAgent` exactly, widened
...
async function reallyLaunchAgent(item, toolId) {
  ...
  // that word: a name ScalAI merely assembled has to stay weak enough for the
...
function toolListHtml(item) {
  ...
  return `<div class="tool-list">${rows}
    <div class="tool-foot">Copies are regenerated from <b>agents/${esc(item.slug)}.md</b>.
      Fichiers without ScalAI's marker are somebody's hand work and are never touched.</div></div>`;
}
...
function renderAgentPickerSheet() {
  ...
  input.addEventListener('keydown', (e) => {
    if (e.key === 'Enter') {
      ...
      else if (a) toast(`Aucun agent installé ne peut exécuter ${a.slug}.`);
    }
  ...
  groups.forEach((g, gi) => {
    ...
    g.forEach((a) => {
      ...
      row.title = tool ? `Start a session as ${a.slug} on ${toolNameOf(tool)}` : 'Aucun agent installé ne peut exécuter cet agent';
    ...
    : '<span>no tool for it</span>'}</span>
      ...
      row.onclick = (e) => {
        ...
        else toast(`Aucun agent installé ne peut exécuter ${a.slug}.`);
      };
      list.appendChild(row);
      if (o.open === a.slug) {
...
// write to (receivers.mjs). Compétences are announced in AGENTS.md; agents are copies.
...
function renderOverlay() {
  ...
  if (o.type === 'agent-compare') return renderAgentCompare();
  ...
  if (o.type === 'settings') return renderParamètres();
  if (o.type === 'quickstart') return renderQuickStart();
}
function closeOverlay() { S.overlay = null; renderOverlay(); }

// ===========================================================================
//  Paramètres — the one place the app explains how it behaves
// ===========================================================================
const SET_SECTIONS = [
  { id: 'voice', name: 'Voix', lead: 'comment ScalAI vous entend' },
  { id: 'look', name: 'Apparence', lead: 'comment ScalAI s’affiche sur ce bureau' },
  { id: 'keys', name: 'Clés', lead: 'clés des agents autorisés et de la voix' },
  { id: 'shortcuts', name: 'Raccourcis', lead: 'les raccourcis qui facilitent votre travail' },
  { id: 'browser', name: 'Navigateur', lead: 'navigateur utilisable par vos sessions' },
  { id: 'usage', name: 'Utilisation', lead: 'consommation par compte connecté' },
  { id: 'notifications', name: 'Notifications', lead: 'quand ScalAI doit vous prévenir' },
  { id: 'about', name: 'À propos', lead: 'à propos de cette version de ScalAI' },
];
function openParamètres(section) {
  rememberHelpFocus();
  ...
  refreshSttInfo().then(() => { if (isParamètresOpen()) renderOverlay(); });
  api.settingsGet().then((s) => { if (isParamètresOpen()) { S.overlay.saved = s; renderOverlay(); } });
}
function isParamètresOpen() { return !!S.overlay && S.overlay.type === 'settings'; }

function renderParamètres() {
  const o = S.overlay;
      ...
      <div class="set-pane" id="set-pane">${
        sec.id === 'voice' ? voicePaneHtml()
          : sec.id === 'look' ? lookPaneHtml()
            : sec.id === 'browser' ? browsers.settingsHtml() : sec.id === 'usage' ? usagePaneHtml() : sec.id === 'notifications' ? notificationsPaneHtml() : sec.id === 'about' ? aboutPaneHtml() : sec.id === 'shortcuts' ? shortcutsPaneHtml() : keysPaneHtml()}</div>
    </div></div>
    <div class="modal-foot">${sec.id === 'voice' ? voiceFootHtml() : sec.id === 'shortcuts' ? '<span class="note">⌘ Command · ⌥ Option · ⇧ Shift</span><button class="shortcuts-link" id="shortcuts-guide">Full guide ↗</button>' : '<span class="note">Enregistrerd on this Mac only, nothing syncs.</span>'}
  ...
  q('#set-done', modal).onclick = async () => { await saveVoiceDraft(modal); if (S.overlay?.saved && Object.prototype.hasOwnProperty.call(S.overlay.saved, 'notificationsEnabled')) await api.settingsSet({ notificationsEnabled: !!S.overlay.saved.notificationsEnabled }); closeOverlay(); };
  if (sec.id === 'voice') wireVoicePane(modal);
  if (sec.id === 'look') wireLookPane(modal);
  if (sec.id === 'keys') wireKeysPane(modal);
  if (sec.id === 'notifications') wireNotificationsPane(modal);
  if (sec.id === 'about') wireAboutPane(modal);
  if (sec.id === 'browser') browsers.wireParamètres(modal);
...
// Paramètres and Quick Start are keyboard-accessible help surfaces. Preserve the
// focused control across async Paramètres refreshes and return to the invoker.
function wireHelpDialog(modal) {
  ...
  modal.tabIndex = -1;
  q('.ov-x', modal).setAttribute('aria-label', 'Fermer dialog');
...
function shortcutsPaneHtml() {
  ...
  return `<h1 class="shortcuts-title">Shortcuts &amp; gestures</h1>
    ...
    <div class="shortcuts-hero">${helpIcon('link')}<div><h2>Ouvrir ce que votre agent crée.</h2>
    ...
    <p class="shortcuts-note">Shortcuts inside an agent’s terminal can vary by agent. This reference covers ScalAI’s controls.</p>`;
}

function notificationsPaneHtml() {
  const enabled = oSettingsValue('notificationsEnabled', true);
  return `<div class="settings-choice">
    <button class="theme-opt set-opt${enabled ? ' picked' : ''}" id="notifications-toggle" type="button" aria-pressed="${enabled}">
      <span class="theme-dot"></span><span class="set-opt-col"><span class="theme-name">Notifications système</span>
      <span class="set-opt-desc">Prévenir quand un agent termine ou rencontre un problème pendant que ScalAI n’est pas au premier plan.</span></span>
      <span class="set-flag ${enabled ? 'ok' : 'wait'}">${enabled ? 'activées' : 'désactivées'}</span>
    </button>
  </div>
  <p class="setup-note">Aucune notification n’est affichée pour une action normale déjà autorisée. Les notifications concernent uniquement les fins de session en arrière-plan.</p>
  <button class="btn" id="notifications-test" ${enabled ? '' : 'disabled'}>Tester la notification</button>`;
}
function oSettingsValue(key, fallback) {
  const saved = S.overlay && S.overlay.saved;
  return saved && Object.prototype.hasOwnProperty.call(saved, key) ? saved[key] : fallback;
}
function wireNotificationsPane(modal) {
  const toggle = q('#notifications-toggle', modal);
  const test = q('#notifications-test', modal);
  toggle.onclick = () => {
    const next = !oSettingsValue('notificationsEnabled', true);
    S.overlay.saved = { ...(S.overlay.saved || {}), notificationsEnabled: next };
    renderOverlay();
  };
  test.onclick = async () => {
    const res = await api.notificationsTest();
    toast(res?.ok ? 'Notification envoyée.' : (res?.error || 'Notifications indisponibles.'));
  };
}
...
function voiceFootHtml() {
  ...
  return `<button class="btn" id="set-mic" ${active && active.ready ? '' : 'disabled'}>◉ Test the mic</button>
    <span class="set-result" id="set-result">${esc(o.test || 'say something and ScalAI will type it back')}</span>`;
}
function voiceFlag(p) {
  // Ready on a key ScalAI never saved means the key arrived on the environment
  ...
  if (p.ready && p.needsKey && !p.keyEnregistrerd) return 'prêt · depuis votre shell';
...
function voiceRowBodyHtml(p) {
  ...
  // Usable, but on a key ScalAI is not holding. The row says where it came from
  // and what would make it survive the next launch.
  if (p.needsKey && p.ready && !p.keyEnregistrerd) {
    return `<div class="set-opt-body"><div class="setup-note">Working from ${esc(p.keyEnv)} in the environment ScalAI was started in.
        Open ScalAI from the Dock and it will not be there.
        <span class="sv-help go-keys" data-keyenv="${esc(p.keyEnv)}">Enregistrer it in Keys</span> to make it stick.</div></div>`;
  }
...
function wireVoicePane(modal) {
  ...
  if (dl) dl.onclick = async () => {
    dl.disabled = true; dl.textContent = 'Téléchargement…';
    ...
    if (isParamètresOpen()) renderOverlay();
  };
  const mic = q('#set-mic', modal);
  if (mic) mic.onclick = () => toggleParamètresMic(modal);
}
...
function toggleParamètresMic(modal) {
...
function lookPaneHtml() {
  return `<div class="field-label">appearance</div>` + THEME_OPTIONS.map((t) =>
    `<button class="theme-opt set-opt${currentThème() === t.id ? ' picked' : ''}" data-theme-id="${t.id}" aria-pressed="${currentThème() === t.id}">
      <span class="theme-dot"></span>
...
function wireLookPane(modal) {
  modal.querySelectorAll('[data-theme-id]').forEach((b) => {
    b.onclick = () => {
      ...
      setThème(theme);
      // setThème rebuilds Paramètres; keep keyboard navigation on the new row.
...
// then have to search. Kept next to REPO_URL so every outward link ScalAI has is
// read in one place.
const DOCS = {
  home: '',
  start: '',
...
//
// ScalAI has no telemetry and is not getting any — "nothing leaves your Mac" is
...
const makerUrl = () => '';
const teamsUrl = (where) => `https://scalai.local/?utm_source=scalai-app&utm_medium=${where}&utm_campaign=teams`;
...
function aboutLine(a) {
  ...
  if (a.state === 'update') return { dot: 'new', text: `ScalAI ${a.latest} is out`, act: 'Download', get: a.url };
...
function aboutPaneHtml() {
  ...
  return `<div class="ab-name">ScalAI${version ? ' ' + esc(version) : ''}</div>
    ...
    <div class="ab-star">
      <button class="btn btn--go" data-url="${REPO_URL}">★ Star ScalAI on GitHub</button>
    </div>
    <div class="ab-links">
    ...
    <hr class="ab-rule" />
    <div class="ab-made">Conçu par <a class="ab-link" href="#" data-url="${makerUrl('about')}">sacalai.vatsk.rub</a>, dans ScalAI.</div>
    <div class="ab-copy">© 2026 ScalAI · Apache 2.0</div>
    <div class="ab-team">
      <button class="btn btn--quiet" data-url="${teamsUrl('about')}">Want ScalAI for your team? →</button>
    </div>`;
}
function wireAboutPane(modal) {
  ...
  act.onclick = async () => {
    ...
    // land there, so closing Paramètres does not lose sight of it.
    ...
    if (isParamètresOpen()) { S.overlay.about = res || { state: 'offline' }; renderOverlay(); }
  };
}

// ---- Models ----------------------------------------------------------------
// ---- Keys — named secrets for permitted agents and Voice -------------------
// Enregistrerd keys override inherited values only for explicitly permitted consumers.
...
function keysPaneHtml() {
  ...
  return `<p class="setup-copy">Enregistrerd keys are shared only with permitted agents. Terminals and installers receive no known API keys from ScalAI. Voice reads its provider keys from this same store.</p>
    ${rows.join('')}
...
function refreshKeys() {
  return api.keysGet().then((res) => {
    if (isParamètresOpen()) { S.overlay.keys = res; renderOverlay(); }
  });
}
function wireKeysPane(modal) {
  ...
  modal.querySelectorAll('.key-row .k-act').forEach((b) => {
    ...
    b.onclick = async () => {
      ...
      else if (act === 'remove') { o.reveal = null; await api.keysSupprimer(name); toast(`${name} removed.`); refreshKeys(); refreshSttInfo(); }
    };
  });
  ...
  const newEnregistrer = q('#key-new-save', modal);
  if (newEnregistrer) {
    const doNew = () => {
...
function renderPeek() {
  ...
  const wrap = document.createElement('div'); wrap.className = 'overlay'; wrap.onclick = requestFermerPeek;
  ...
  box.innerHTML = `<div class="peek-head">
      ...
      <button class="t-btn pk-x" title="Fermer"><span class="uni-i">✕</span><span class="pix-i">${pixIcon('close')}</span></button>
  ...
  q('.pk-x', box).onclick = requestFermerPeek;
}
...
function renderConnectCatalog() {
  ...
  const modal = overlay('picker-box', `<div class="picker-input"><span class="prompt-mark">⚡</span>
    ...
    ${cat.length ? '' : '<div class="rail-empty" style="padding:14px">Chargement du catalogue…</div>'}
    <div class="svc-grid">${cat.map((s) => `
      <div class="svc-card${connectedIds.has(s.id) ? ' connected' : ''}" data-id="${esc(s.id)}" tabindex="0">
        ...
        ${s.id === 'kie' ? '<span class="sv-by">par ScalAI</span>' : ''}
...
function renderConnectForm() {
  ...
  const modal = overlay('setup-box', `
    ...
    ${guided
      ? `<p class="setup-copy">${esc(svc.guide)}</p><div class="ni-agent">${chosenAgent(o)
          ...
          : 'No agent is installed yet. Press ⌘N to add one first.'}</div>`
      : folder
        ? `<p class="setup-copy">Pick the one folder your agents may read and edit. Nothing outside it is reachable.</p><button class="btn" id="sv-pick-folder">Choisir un dossier…</button><div class="setup-note" id="sv-folder-note">${esc(o.values.folder ? shortHome(o.values.folder) : '')}</div>`
  ...
  if (install && o.installed === undefined) {
    q('#sv-connect', modal).textContent = 'Installer d’abord';
    api.statPath({ token: installDirOf() + '/dist/index.js' }).then((st) => {
      o.installed = !!(st && st.exists);
      const b = q('#sv-connect', modal);
      if (b && b.textContent !== 'Connecting…') b.textContent = o.installed ? 'Connect' : 'Installer d’abord';
    });
  } else if (install) {
    q('#sv-connect', modal).textContent = o.installed ? 'Connect' : 'Installer d’abord';
  }
  q('#sv-connect', modal).onclick = async () => {
    ...
    if (svc.keys.some((k) => !o.values[k.id]) || (folder && !o.values.folder)) { toast(folder ? 'Choose a folder first.' : 'Collez d’abord votre clé.'); return; }
...
function renderConnectCustom() {
  ...
  q('#svc-go', modal).onclick = () => {
    ...
    closeOverlay();
    // The agent registers into the master; ScalAI fans it out when the session
    ...
    agentSession(w, { title: 'build: connector', code: 'BC', seed:
      `Build an MCP connector for this: ${o.text.trim()}. When it works, register it for this project by adding one entry to connections.json at the project root, under the standard "mcpServers" key (create the file if it is missing) — ScalAI copies it to every installed agent's own config from there. Then tell me what tools it exposes.`, onExit });
...
// ScalAI had no such place: the Help menu is five outbound links, and the person
  ...
  return [
    {
      ...
      sub: 'ScalAI only ever looks inside it. No folder yet? It will make you one.',
      done: !!S.project,
      acts: S.project ? [] : [{ label: 'Make me a folder', go: true, run: () => { closeOverlay(); makeFolderDialog(); } }],
    },
    {
      n: 2, title: 'Press Nouvelle session and pick who runs it',
      sub: 'The list shows what is on your Mac. Anything missing installs from the same list.',
      acts: [
        { label: 'Nouvelle session ⌘N', go: true, run: () => { closeOverlay(); openLauncher(); } },
        { label: '▶ Watch · 2 min', play: 'getting-started' },
      ],
    },
    {
      n: 3, title: 'ScalAI can run multiple agents for you',
      sub: 'Claude Code signs in with your Claude account, Codex with your ChatGPT one. No ScalAI account, no second bill.',
    ...
    },
    {
      ...
      sub: 'Une carte ambre « Votre autorisation est requise » signifie que l’agent attend votre réponse. Rien ne se passe sans vous.',
      acts: [{ label: 'How permissions work', run: () => api.openUrl(DOCS.permissions) }],
    },
    {
      n: 6, title: 'Ouvrir ce que votre agent crée',
      sub: OPEN_OUTPUT_COPY,
      acts: [{ label: '⌘ Shortcuts & gestures', run: () => openParamètres('shortcuts') }],
    },
  ];
}
...
function overlay(cls, inner, opts) {
  ...
  const x = document.createElement('button'); x.className = 't-btn ov-x'; x.title = 'Fermer'; x.innerHTML = `<span class="uni-i">✕</span><span class="pix-i">${pixIcon('close')}</span>`; x.onclick = closeOverlay;
  modal.appendChild(x);
  wrap.appendChild(modal); els.overlayRoot.appendChild(wrap); return modal;
}

// ---- folders ---------------------------------------------------------------
// A file opened with ScalAI from Finder. Either it already lives on this desk —
...
// Enregistrer → clear → restore, in that order. The save has to name the *outgoing*
// folder explicitly: savePanels() reads S.project, which is about to change.
async function swapDesk(info) {
  for (const p of S.panels) if (p.kind === 'browser' && browsers.hasPending(p) && !await browsers.canFermer(p)) return;
...
async function restoreDeskFor(folder) {
  let snaps = [];
  let meta = null;
  try { [snaps, meta] = await Promise.all([api.loadPanels(folder), api.loadDeskMeta?.(folder)]); } catch (_) {
    try { snaps = await api.loadPanels(folder); } catch (_) { snaps = []; }
  }
  if (Array.isArray(snaps) && snaps.length) await restorePanels(snaps);
  else renderAll();
  if (meta && S.panels.length) {
    const restored = restoreDeskMeta(S.panels, meta);
    S.activeId = restored.activeId;
    S.expandedId = restored.expandedId;
    if (restored.view === 'split' && S.panels.length > 1) {
      S.view = 'split';
      S.split = splitAfter({ ...S.split, panels: S.panels }, { type: 'enter', activeId: S.activeId });
      applyViewAttrs();
    }
    renderAll();
  }
...
// ScalAI has to say — so it waits, and "Not now" means not this version, ever.
...
function paintUpdate(state, ev) {
  ...
  if (state === 'downloading') {
    ...
    els.updateRoot.innerHTML = `<div class="update-note">
      ...
      <span class="un-msg">getting ScalAI ${version}…</span>
  ...
  if (state === 'ready') {
    ...
    // takes its time closing, Squirrel waits for it, and reopening ScalAI inside
    ...
    els.updateRoot.innerHTML = `<div class="update-note">
      ...
      <span class="un-msg">ScalAI ${version} is ready</span>
  ...
  // The one warning this feature owes anybody. Installing restarts ScalAI, and
  ...
  els.updateRoot.innerHTML = `<div class="update-note">
    <span class="un-dot"></span>
    <span class="un-msg">${broke ? `ScalAI ${version} has to be installed by hand` : `ScalAI ${version} is out`}</span>
...
// ===========================================================================
//  The one time ScalAI asks for anything
// ===========================================================================
// ScalAI is free, and the only thing that helps anyone find it is a star. But the
...
// evidence available that ScalAI earned its place. Nothing is sent anywhere to
...
// launched ScalAI is going somewhere; this waits until they have arrived.
...
function paintStarAsk() {
  ...
  // Green, not amber: amber in ScalAI means *needs you*, and this does not.
  els.updateRoot.innerHTML = `<div class="update-note">
    <span class="un-dot un-done"></span>
    <span class="un-msg un-ask">Enjoying ScalAI? A star helps other people find it.</span>
