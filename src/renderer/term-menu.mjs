...
// like it would copy a hyperlink, and "Copier le chemin" on a url reads like a bug.
function copyRow(kind, value) {
  return { label: kind === 'url' ? 'Copy link' : 'Copier le chemin', copy: value };
}

export function termMenuItems({ kind, text, st }) {
  if (kind === 'url') {
    return [
      { label: 'Ouvrir dans le navigateur', kb: '⌘click' },
      '-',
